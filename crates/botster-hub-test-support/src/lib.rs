...
pub fn write_botster_web_production_fixture(dir: &Path) {
    fs::create_dir_all(dir.join("scripts")).expect("create botster-web package root");
    fs::write(dir.join("plugin.lua"), "return botster.register({})\n")
        .expect("write botster-web core entrypoint");
    fs::write(
        dir.join("scripts").join("local-package-server.mjs"),
        r#"
import fs from 'fs';
