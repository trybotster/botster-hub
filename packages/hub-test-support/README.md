...
Use this command for version 0.1.34:

```sh
npm install --save-dev @trybotster/ui-contract@0.3.2 @trybotster/hub-test-support@0.1.34
```
...
{
  "devDependencies": {
    "@trybotster/hub-test-support": "0.1.34"
  }
}
```

`@trybotster/hub-test-support@0.1.34` carries byte-faithful live
`terminal_output` payloads (`payload_base64`, `payload_encoding`, `bytes`)
and authentic dual GHOSTSNP late-attach fixtures (conformance revision 39).
...
Version 0.1.34 carries protocol version 7 / conformance revision 39 with
advertised optional `unix_terminal_adapter` and
