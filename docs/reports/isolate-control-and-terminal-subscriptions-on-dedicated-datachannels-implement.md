...
## Review-return repairs (`review_1787810573_662899`)
...
- GitHub CI on `26fcbfe` still failed the shutdown proof with READY and FINISH and no Attached. The SlotReady mark was dropped after an empty observe, so later owner turns (including Status) did not retry Attached. Coalesced drain now keeps the SlotReady key until a live bound slot is Full. Empty-slot Pump is rate-limited to 50 ms so that persist does not spin. SlotReady stays a doorbell. Generic Status/ListSessions still do not exact-observe. Local official `./test.sh --locked`: exit 0, hub lib 529, lifecycle 320 passed 2 ignored (310.78 s). GitHub CI on this head is the remaining gate.
...
- GitHub CI on `26fcbfe` was still red on that shutdown proof. Local official after persisting empty SlotReady until the slot fills is green. Implement gate stays held until GitHub CI is green.
