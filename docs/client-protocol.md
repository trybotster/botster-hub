...
control path and sends one snapshot. If the encoded frame exceeds the daemon
frame limit, Hub returns `entity_provider_frame_too_large` instead of a
partial baseline. `entity_upsert`, sparse `entity_patch`, and
