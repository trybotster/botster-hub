...
Rust clients consume crates.io `botster-ui-contract = "0.3.2"`; TypeScript
clients consume the prepared `@trybotster/ui-contract@0.3.2`. Those packages
are one contract version. Publish them together with `script/publish-npm-packages`.
The generated declarations, schema, and conformance fixtures are one contract
surface and must not be copied into a client repository. Do not pin the Rust
crate from a Hub Git SHA.
...
library. The UI contract is not a Hub Git identity: depend on crates.io
`botster-ui-contract = "0.3.2"`. Git `botster-hub-client` and
`botster-hub-test-support` already resolve that registry crate; do not add a
second Hub-git `botster-ui-contract` pin.
