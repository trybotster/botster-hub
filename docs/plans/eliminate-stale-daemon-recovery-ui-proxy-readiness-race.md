...
- Project Pipelines current context: both prior plan artifacts, failed Implement report `artifact_1784236909_385350`, approved reviews `review_1784236401_958678` and `review_1784237692_485524`, the latter review's four findings, human answers `question_1784236756_337513` and `question_1784238031_807799`, the returned Plan step, and no remaining unanswered question or blocking dependency.
...
- Merged constraint: ticket `ticket_1784168174_908058`, PR #140, commit `01549e9`, and `docs/plans/eliminate-loaded-dev-stack-restart-teardown-race.md`. Its safety premise remains binding: an exiting daemon must not reclaim a pathname removed for replacement or unlink a replacement-owned socket.
...
- Current transport: `serve_daemon` owns a plain `UnixListener`; shutdown calls unconditional `cleanup_socket_path`; the active loop notices a missing pathname but calls a no-op `rebind_missing_socket_path`. `prepare_socket_path` uses a protocol probe to preserve a live daemon, removes stale/unusable paths, then startup binds.
...
8. Verify immediate incompatible-daemon replacement, PR #140 restart paths, default-parallel lifecycle coverage, mechanism-specific deterministic red-on-revert, and the loaded residual-tail campaign with unchanged timeouts/concurrency.
...
- `src/main.rs` — investigation and wiring surface. Temporary scoped instrumentation may identify `recover_owned_stale_dev_stack_daemon` as the pathname actor; retain a production edit here only if the trace and named-test revert prove it is the actual fix. `start_botster_web_dogfood` consumes `/health` and UI before `local_runtime_up` prints readiness.
...
- `docs/plans/eliminate-loaded-dev-stack-restart-teardown-race.md` — unchanged merged constraint source.
...
10. Produce separate red proof for each behavior that ships: cached-existence health makes the stale-file health test red; reverting the actor-driven recovery fix makes the named `up` path red for the original cause; reverting a shipped ownership guard makes the forced-interleaving test red. Restore all changes before commit and run identical loaded verification green.
...
- **False green under load:** focused ownership tests cannot replace the default-parallel 20-run campaign. Preserve exact first-red evidence and investigate unrelated failures.
...
1. Deterministic actor characterization: run the named recovery test with scoped instrumentation and attach the ordered removal/bind/rebind trace naming the actual pathname actor. Evidence must distinguish old daemon cleanup, `recover_owned_stale_dev_stack_daemon`, `prepare_socket_path`, active repair, and any external remover; "unknown" requires exact instrumentation attempted and a new stop-and-ask, not both speculative fixes.
...
5. PR #140 regression paths remain immediate and green:
   - `./test.sh --test hub_daemon_lifecycle_test cli_dev_stack_bootstrap_reuses_live_daemon_and_preserves_state_after_restart -- --exact --nocapture`
   - `./test.sh --test hub_daemon_lifecycle_test cli_local_runtime_up_starts_reuses_and_down_stops_runtime -- --exact --nocapture`
6. Neighboring packaged launcher/bridge tests, including `cli_dogfood_launcher_starts_botster_web_in_existing_hub_mode_and_shuts_down`, `cli_dogfood_launcher_bridge_request_endpoint_uses_same_daemon_state`, and the unchanged health-only rejection test.
...
10. Loaded acceptance: exact restored subject SHA through the existing runner with `test_target=lifecycle-suite`, `stress_profile=residual-tail`, at least 20 repetitions, default Cargo parallelism, unchanged budgets, first-red stop, resource samples, and clean owned-process teardown. No 502, WebHealth timeout, permanent ENOENT, or unrelated red is waived.
...
- Verify must include the focused actor-driven repair, conditional ownership, health, and runtime tests; workspace checks; the default-parallel lifecycle suite; and 20-run loaded evidence with exact attribution.
