...
- `src/daemon_transport.rs` — deliberate behavioral consumer: normalized request-read disconnects take the existing subscription-detach path, and a normalized hello-write disconnect in `prepare_socket_path` falls through to the existing stale-socket cleanup rather than aborting startup.
...
4. Cover the daemon's existing `ClientDisconnected` request-read branch detaching active subscriptions, and make `prepare_socket_path` deliberately fall through when its hello write returns the normalized disconnect semantic. Keep all other probe write errors fatal.
...
- **Daemon-side behavior change:** normalized request-read disconnects now detach subscriptions cleanly, and normalized hello-write disconnects no longer abort startup. Focused daemon coverage and preservation of unrelated fatal errors keep both effects deliberate.
...
- Daemon disconnect cleanup: `./test.sh --lib daemon_transport::tests::client_disconnected_read_detaches_connection_subscriptions -- --exact --nocapture` proves the normalized semantic reaches the daemon's clean subscription-detach path.
