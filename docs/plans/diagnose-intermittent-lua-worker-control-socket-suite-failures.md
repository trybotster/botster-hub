...
- Project Pipelines context: ticket `ticket_1784506652_117691`, run `run_1784592808_567657`, active Plan step `run_step_1784592808_723262`, required gate `botster_plan_gate`, and no current-run artifacts, findings, reviews, questions, answers, or blocking dependencies.
...
- Workflow discipline: run checklists `checklist_1784592886_900127` (vault) and `checklist_1784592891_396191` (planning workflow) own the note coverage, convention-fit, verification, capture, runtime-trace, artifact, and gate evidence for this step. Both checklist creation calls timed out at the worker boundary but committed; listing before retry found them and avoided duplicates.
...
- `script/run-loaded-daemon-lifecycle`: add one bounded `hub_lua_runtime_test` selector using the unchanged default-parallel test binary and existing first-red/process-group cleanup behavior.
- `.github/workflows/loaded-daemon-lifecycle.yml`: expose the selector and precompile the selected integration test binary rather than always precompiling only `hub_daemon_lifecycle_test`.
- `docs/loaded-daemon-lifecycle-runner.md`: document the focused Lua-worker target as diagnostic evidence, not a replacement for full-suite acceptance.
...
1. Extend the existing loaded harness with a `focused-lua-worker-suite` target that precompiles and runs `./test.sh --test hub_lua_runtime_test -- --nocapture` at default test concurrency under the selected bounded stress profile. Preserve first-red stopping, exact SHA validation, complete logs/resource samples, and owned-process teardown.
2. Run a fixed-SHA diagnostic campaign against an exact committed Hub revision. On the first red, do not rerun before saving the artifact. Extract the first non-cascade failure, actual test name, error chain, worker/socket/process facts, and cleanup result. If later failures are consequences of a shared panic or leaked process, label them cascade evidence rather than independent roots.
3. Add the narrowest bounded diagnostic at the owner that can distinguish child lifecycle branches. The parent must retain or report bounded worker stderr and child status on startup failure without leaking paths, environment, payloads, or secrets; successful startup remains quiet. Prove the diagnostic path with deterministic worker exit and delayed/readiness fixtures before relying on another loaded recurrence.
...
6. After the core fix is merged, update Hub `Cargo.lock`, run the two exact Lua caller tests, the whole default-parallel `hub_lua_runtime_test` binary, adjacent worker adoption/restart coverage, strict formatting/lints, and `./test.sh`.
7. Run fixed-SHA loaded proof for the focused Lua target. Require a meaningful repetition campaign with unchanged concurrency/assertions and clean teardown; then run at least one ordinary full default-concurrency suite on the same SHA. A green focused campaign alone is not resolution.
8. Update this artifact and Project Pipelines with the exact diagnosis, upstream ticket/commit, red-on-revert evidence, commands, repetitions, full-suite result, and any remaining unverified tail. Capture a vault note only if the result establishes a reusable control-socket readiness/lifecycle rule.
...
- Loaded gate: the exact fixed Hub SHA completes the focused Lua-worker campaign under the selected bounded stress profile for the approved repetition count with unchanged assertions/concurrency and `cleanup_status=0`. The implementation report must state what load was actually observed.
