...
- Whole-suite contention gate: if the full Lua-binary campaign completes without the control-socket failure, preserve a bounded default-concurrency `./test.sh` campaign under the same `residual-tail` profile, with first-red attribution and cleanup evidence. This gate may establish non-reproduction under the required topology; it does not by itself manufacture a deterministic regression or justify a resolved claim.
...
- Disposition: the control-socket defect is not reproduced under the targeted 20-run subset budget and is not resolved. The sibling has now landed, so this fresh run must integrate current main, rerun the full Lua-binary campaign, and—if still green—execute the required whole-suite contention-topology escalation.
