...
2. Run a fixed-SHA diagnostic campaign against an exact committed Hub revision. On the first red, do not rerun before saving the artifact. Extract the first non-cascade failure, actual test name, error chain, worker/socket/process facts, and cleanup result. If later failures are consequences of a shared panic or leaked process, label them cascade evidence rather than independent roots.
   The approved bounded campaign is 20 repetitions with `residual-tail`; 20/20 green means only "not reproduced under this focused budget." In that case, escalate to whole-suite contention (looped default-concurrency `./test.sh` or the focused binary running alongside equivalent full-suite contention) before concluding reproduction is exhausted.
   Run `29790592442` stopped first-red at repetition 8 on the distinct `plugin_db_missing_get_returns_absent_record_shape_and_preserves_success_shape` timeout while both worker-backed session-template callers passed. Human answer `question_1784595267_687574` kept that root out of this ticket, assigned it to blocking sibling `ticket_1784595368_465499`, and required the full Lua campaign to rerun after the sibling lands. Until then, `focused-lua-session-worker-callers` may continue bounded control-socket reproduction over the unchanged `session_template` subset at default parallelism; it is diagnostic-only and cannot satisfy full-suite acceptance.
...
- Blocking sibling gate: `ticket_1784595368_465499` must land before the interrupted full Lua-binary campaign is rerun. A diagnostic-only `focused-lua-session-worker-callers` result may inform control-socket reproduction but cannot satisfy the loaded gate.
