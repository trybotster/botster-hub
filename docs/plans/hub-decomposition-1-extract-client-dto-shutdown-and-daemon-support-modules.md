...
19. `grep -rn "hub_source(" tests/` enumerates every named-file source-scanning guard, and each scanned string still lives in the file that guard names. Also enumerate `include_str!` production source lists in `src/`, especially `production_sources_reject_terminal_drain_and_snapshot_phase_decode`. A guard whose scan list still names only `daemon_transport.rs` after a move is a coverage hole, not a green proof.
...
25. After the move, `production_sources_reject_terminal_drain_and_snapshot_phase_decode` includes every new file under `src/client_api_dto/` and `src/daemon/`, and `no_lua_dispatch_in_terminal_input_or_output` walks `src/` recursively. Ablation: a production `GHOSTSNP` line in `src/client_api_dto/session.rs` must fail the lib.rs guard; a `lua_runtime` marker in that same file must fail the lua-dispatch guard. Remove the ablation before commit. This check is a follow-up to the move-only commit: the move itself stays one commit, and this coverage restore is a separate later commit required when Verify proves the old scan lists no longer reach the moved mapper.
...
6. A note that Hub carries source-scanning guard tests that assert on Hub source text through `hub_source()` **and** through `include_str!` lists plus directory walks. Any Hub decomposition ticket must enumerate every such assertion before moving code. A hardcoded file list that still names only the old module after a move stays green while the moved production lines go unscanned.
