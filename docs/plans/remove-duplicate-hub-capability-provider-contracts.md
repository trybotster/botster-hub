...
- Repo context inspected: `src/lib.rs`, `src/packages.rs`, `src/providers.rs`, `src/core.rs`, `src/adapters/{clients,cloud,signaling,api}.rs`, `src/adapters/mod.rs`, `src/main.rs`, `README.md`, `Cargo.toml`, and `test.sh`.
...
- Remove hub-only duplicate capability vocabulary where it competes with current core package/capability contracts:
  - Delete `ProviderCapability` as a hub-owned enum.
  - Delete `CapabilityGrant` if it remains only a wrapper around the duplicate hub enum and has no production caller.
- Remove dead placeholder provider/core/adapter/package enums with no real behavior. Apply one consistent rule to every placeholder enum in the touched modules: delete it, or keep it only if implementation adds a real caller and a stated rationale in this run. Default to deletion for:
  - `EmbeddedCoreRole`
  - `PackagePolicy`
  - `AdapterFamily`
  - `ClientTransport`
  ...
  - `SignalingContract`
  - `ApiContract`
- Update public facade and smoke output so the crate no longer advertises “provider capability contracts” as hub-owned vocabulary. Explicitly delete `ArchitectureSummary::provider_capabilities`, the `provider_capabilities` field, and `PROVIDER_CAPABILITIES`; do not rebuild a decorative parallel list keyed on `botster_core::Capability` or `CapabilitySurface`.
...
- Assumption: because the duplicate and placeholder enums have no callers beyond docs/tests/smoke text, they can be deleted cold turkey without a compatibility bridge.
- Assumption: `PackagePolicy`, `AdapterFamily`, and `ClientTransport` are definition-only label taxonomies, not real admission/lifecycle/product policy behavior; delete them unless implementation creates a real caller and rationale in this run.
- Assumption: a small hub-side struct that pairs a `botster_core::Capability` with a hub policy reason is acceptable only if it has a real caller or test-proven rationale; otherwise it is the same docs-as-code problem under a core-typed name.
- Unknown for implementer to confirm before editing: whether deleting `src/providers.rs`, `src/core.rs`, `src/adapters/clients.rs`, and the placeholder adapter modules leaves any module export or doctest references behind.
...
- `src/lib.rs`: public facade, `ArchitectureSummary`, doctest, tests, responsibility wording, and removal of `provider_capabilities` facade members.
- `src/main.rs`: smoke-path summary text and any count based on duplicate provider capability contracts.
- `src/packages.rs`: remove `PackagePolicy` and remove or replace `CapabilityGrant`/`ProviderCapability` with current core package/capability contracts only if a real caller exists.
...
- `src/adapters/clients.rs`, `src/adapters/cloud.rs`, `src/adapters/signaling.rs`, `src/adapters/api.rs`, and `src/adapters/mod.rs`: delete or reduce to real adapter namespaces only if callers remain; default to deleting zero-caller placeholder enums and module docs that only justify them.
- `README.md`: responsibility split, crate layout, scaffold exclusions, and dependency policy wording that currently implies hub-owned provider capability vocabulary. The crate-layout table must match the post-deletion module set.
...
- Weak runtime proof: removing docs-as-code can make tests pass while behavior remains unproved. This crate is intentionally scaffold/facade-only at this stage; `tests/hub_runtime_test.rs`, not the `src/main.rs` print string, is the primary behavioral proof because it exercises `HubRuntime` through current `DefaultBotsterEngine`.
...
- `rg -n "ProviderCapability|CapabilityGrant|EmbeddedCoreRole|PackagePolicy|AdapterFamily|ClientTransport|CloudContract|SignalingContract|ApiContract" src README.md tests` returns no stale duplicate/placeholder vocabulary unless a remaining occurrence has a real caller and explicit rationale in the implementation report.
...
- README positively states that `botster-core` owns package manifests, `Capability`, `CapabilitySurface`, host-profile admission contracts, and capability runtime primitives, and that `botster-hub` owns product host policy over those core contracts.
- README crate-layout table no longer lists deleted files/modules such as `src/providers.rs` or `src/adapters/*` if implementation removes them.
- `src/lib.rs` no longer exposes `ArchitectureSummary::provider_capabilities`, `PROVIDER_CAPABILITIES`, or an equivalent decorative capability list without a real caller.
...
- Runtime proof: `tests/hub_runtime_test.rs` remains the primary behavioral evidence and still proves spawn, attach, write, drain, classify, and shutdown through `HubRuntime` -> `DefaultBotsterEngine`. `src/main.rs` is smoke output only and should still construct `HubRuntime::new(config)` without duplicate provider capability counts.
