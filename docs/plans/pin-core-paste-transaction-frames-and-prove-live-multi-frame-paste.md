...
- Hub ingress path: `src/transport/shared/ingress.rs` (`IngressBuffer`, `sync_channel(MIN_ADAPTER_INGRESS_BUFFER_FRAMES)`, header validation only, `Lost` latch), `src/transport/shared/adapter_slot.rs::push_ingress`, Unix caller `src/transport/unix/connection.rs` (`UnixInbound::Terminal` → `handle.push_ingress`), WebRTC caller `src/transport/webrtc/subscription_channel.rs::run_bound_terminal_channel` (one AES-GCM envelope per DataChannel message → `handle.push_ingress`).
- Frame size headroom: Unix mux frame cap is `DAEMON_MAX_FRAME_BYTES = 1 MiB` (`src/admission/budgets.rs`), so a 65,539-byte frame base64-wrapped in `DaemonUnixTerminalEnvelope` (~87.5 KB JSON line) fits. WebRTC: locked `rtc-sctp 0.21.0-beta.2` default `max_message_size = 262,144`, so one ~88 KB encrypted envelope fits in one DataChannel message. Hub sets no custom SCTP message size.
...
- Any Hub-side paste state, chunk policy, reordering, acknowledgement, retry, timeout, or decode. Hub stays content blind.
...
- **botster-hub** owns concrete Unix and WebRTC adapters, header validation, the bounded opaque ingress buffer, admission, and route lifecycle. This ticket changes none of those mechanisms; it proves them under 19-frame bursts.
...
No production `src/**` file changes are expected. Any production diff must return to planning.
