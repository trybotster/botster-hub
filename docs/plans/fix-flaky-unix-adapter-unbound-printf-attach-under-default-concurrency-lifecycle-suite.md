...
Unknown: whether another lifecycle-suite test flakes during the acceptance runs. If one does, register a new ticket with exact evidence. Do not expand this repair mid-run.
...
- The lifecycle suite may flake on an unrelated test during acceptance runs. Follow the prior-art rule: exact evidence or a new ticket; do not absorb.
...
3. Binding default-concurrency gate: `./test.sh --locked --test hub_daemon_lifecycle_test` (full lifecycle binary, default test threads) passes 5 consecutive runs with zero failures. The pre-existing 1 ignored test stays ignored.
