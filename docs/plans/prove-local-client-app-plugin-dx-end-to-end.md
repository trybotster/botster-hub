...
- Ticket: "Prove local client app plugin DX end to end". Acceptance asks for docs/tests proving fresh-checkout commands, stable data-dir persistence of local package installs, obvious dogfood URL/TUI output, and no PII.
...
- Repo context inspected: `README.md`, `src/main.rs`, `tests/hub_daemon_lifecycle_test.rs`, `tests/hub_local_dogfood_test.rs`, `docs/plans/expose-installed-app-registry-and-structured-app-launch-dtos.md`, `docs/plans/document-and-test-standalone-botster-tui-dogfood-flow-from-hub.md`, and related dogfood/app plan artifacts.
- Current implementation shape:
  - `botster-hub dogfood` can install/enable `botster-web` from a local path and optionally install/enable `botster-tui` from `--tui-package-path`.
  - Dogfood output prints `web=http://127.0.0.1:<port>/?dogfood=real-hub`, `bridge=...`, `tui=botster-hub apps open --data-dir <dir> botster-tui`, MCP/status/shutdown commands, and package state rows.
  - `apps list/show/open` exist and terminal apps are launched through daemon-resolved `ResolveAppLaunch` contracts.
  - Existing tests cover structured web URL, generated data dir, explicit data-dir rerun idempotency, dogfood TUI package enablement plus `apps open botster-tui`, app registry DTOs, structured web open URL, terminal open, and deprecated `botster-hub tui` delegation.
...
- Add or tighten final docs/tests that prove the combined local client app plugin developer experience from a stable local data dir:
  ...
  - list installed apps from the stable dogfood data dir;
  ...
  - preserve stable data-dir package state across dogfood reruns;
  - keep dogfood output path-neutral and free of local package source paths.
- Update `README.md` only where the fresh-checkout command sequence or expected dogfood output is still ambiguous for first-party client package DX.
...
- No broad cleanup of dogfood startup, package lifecycle, entrypoint supervision, or CLI formatting outside the proof path.
...
- Assumption: the production runtime path to prove is `botster-hub dogfood` -> local package install/enable -> daemon `ListApps` / `ResolveAppLaunch` / `StartPackageEntrypoint` -> CLI `apps open`.
...
- Operator documentation for local first-party client dogfood.
...
- `README.md`
  - Tighten the local dogfood operator section so a fresh-checkout operator can run the web/TUI local package proof with stable `--data-dir`.
  - Make expected output lines obvious: `web=.../?dogfood=real-hub`, `tui=botster-hub apps open --data-dir <dir> botster-tui`, and optional `apps list/show/open` commands.
- `src/main.rs`
  - Only touch if implementation discovers dogfood output omits necessary app-list/open guidance, or if `botster-hub tui` no longer visibly delegates through `apps open`.
  - Preserve thin CLI behavior; terminal launch contracts must continue coming from daemon `ResolveAppLaunch`.
- `tests/hub_daemon_lifecycle_test.rs`
  - Primary place to extend end-to-end proof. Likely extend `cli_dogfood_launcher_enables_local_tui_package_for_apps_open` or add a focused neighboring test to:
    - run `dogfood` with both `--web-package-path` and `--tui-package-path`;
    ...
    - assert no local package source paths appear in dogfood/CLI output.
...
- PII/path leakage risk: dogfood output, test assertions, README examples, and plan/report artifacts must not expose local package roots, home paths, or run-specific worktree paths.
...
- Focused end-to-end dogfood proof:
  - `./test.sh --test hub_daemon_lifecycle_test cli_dogfood_launcher_enables_local_tui_package_for_apps_open`
  - Should prove dogfood with stable explicit data dir, local `botster-web` and `botster-tui` package paths, structured `web=` URL, package enablement, `apps open botster-tui`, and no path leakage.
- Add or extend focused app listing/open proof if not already covered by the prior command:
  ...
  - Must prove `apps list/show/open` against the same dogfood data dir after installing both local client packages.
- Existing web dogfood proof:
...
- Documentation/path scan:
  - `rg -n "dogfood|apps list|apps show|apps open|botster-tui|web=|tui=|BOTSTER_HUB_SOCKET|--tui-package-path" README.md docs/client-protocol.md`
...
- Plan Review should send implementation back if it adds new protocol surfaces, proves only code existence without the real dogfood/CLI runtime path, leaks local paths, or documents TUI behavior beyond what the hub-owned fixture/downstream evidence proves.
...
- Capture after implementation if the composed first-party client dogfood command sequence becomes the durable convention: stable explicit data dir, local `botster-web` and `botster-tui` package paths, app list/show/open proof, and path-neutral dogfood output.
...
- The plan follows loaded Botster constraints: hub CLI remains thin, package/app state flows through daemon DTOs, terminal app launch uses daemon-resolved foreground contracts, dogfood uses explicit data dirs and verified runtime output, Project Pipelines remains plugin-owned, and repo-visible artifacts cite vault notes by wikilink rather than local filesystem paths.
