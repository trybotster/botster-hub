...
9. Run the Hub production acceptance harness in `upgrade` mode using the
   ...
   cannot read or mutate operator caches. The Web harness must attach to the
   already-installed current package in that durable state; do not edit
   persisted state, remove/reinstall the package, prune sessions, or issue a
   manual reload to make the proof pass.
