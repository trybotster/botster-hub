...
| 6a | `./test.sh --locked --test session_projection_owner_loop git_visible_hub_members_share_one_exact_core_revision -- --exact` | 1 passed: every Core-family declaration in the three member manifests uses the exact `.git` URL and `REQUIRED_CORE_REV`; README has no ``tracks `botster-core` from the `main` branch`` sentence and contains ``one exact `rev` `` |
| 6b | `./test.sh --locked --test session_projection_owner_loop git_visible_hub_members_reject_one_mixed_core_revision -- --exact` | 1 passed: a fixture with one synthetic invalid rev and one compliant rev fails the per-declaration guard while a whole-file `contains` check would pass |
| 6c | `./test.sh --locked --test session_projection_owner_loop git_visible_hub_members_reject_one_mixed_core_url -- --exact` | 1 passed: a fixture with one URL without `.git` and one compliant URL fails the per-declaration guard |
