# Eliminate the loaded local runtime restart teardown race
...
- Production path: `shutdown` and `down` return after the daemon acknowledges shutdown. An immediate the removed bootstrap command or `up` calls `prepare_local_runtime` -> `ensure_local_runtime_daemon` -> `daemon_transport_request(Status)`. That policy already spawns a replacement for `NotRunning` and `ClientDisconnected`, but the hub-client boundary currently leaves reset/broken-pipe/unexpected-EOF errors as `Io`, causing the catch-all fatal branch.
...
- Route client-boundary socket I/O mappings used by connect/hello, frame writes, cloned-stream setup, and frame reads through that normalization where those three disconnect kinds can surface. Reuse the existing error variant and the existing `ensure_local_runtime_daemon` replacement policy; add no new public enum or startup policy branch.
...
- Preserve the existing immediate, unbarriered `shutdown` -> the removed bootstrap command sequence in `cli_local_runtime_bootstrap_reuses_live_daemon_and_preserves_state_after_restart`.
...
- No change to daemon shutdown response ordering, `daemon.stop()`, socket cleanup, metadata format, readiness probe order, `ensure_local_runtime_daemon` policy, or stale-daemon recovery.
...
- `docs/plans/eliminate-loaded-local runtime-restart-teardown-race.md` — revised Plan artifact.
- `src/main.rs` — read-only wiring evidence: `ensure_local_runtime_daemon` already replaces `ClientDisconnected`; `local_runtime_down`, `local_runtime_bootstrap`, and `local_runtime_up` are the production entry points.
...
5. Leave `ensure_local_runtime_daemon`'s `NotRunning | ClientDisconnected` replacement arm unchanged. Its existing production call chain is the wiring proof that normalized errors spawn a fresh daemon.
6. Keep `cli_local_runtime_bootstrap_reuses_live_daemon_and_preserves_state_after_restart` immediate and unbarriered. Extend the local runtime lifecycle test with immediate `down` -> `up`, fresh-start/readiness assertions, and normal final shutdown.
...
- **Policy duplication:** matching OS kinds again in `ensure_local_runtime_daemon` would make callers diverge. Normalize once in the client crate and reuse `ClientDisconnected`.
...
- Immediate shutdown/bootstrap: `./test.sh --test hub_daemon_lifecycle_test cli_local_runtime_bootstrap_reuses_live_daemon_and_preserves_state_after_restart -- --exact --nocapture` passes without any PID/socket wait, sleep, retry, or serialization.
...
- Runtime wiring review: confirm `ensure_local_runtime_daemon` still consumes `ClientDisconnected` to call `spawn_local_runtime_daemon`, and confirm the integration diff contains no pre-restart completion barrier.
