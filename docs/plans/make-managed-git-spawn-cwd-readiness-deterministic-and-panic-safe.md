...
The replacement readiness oracle will drain only the returned session UUID through the production daemon until its exact `SessionLifecycle { state: "exited" }` event is observed, then require that UUID to remain present with canonical lifecycle `exited` through `ListSessions`. Focused implementation proof established that `ListSessions` alone retains `running` until CoreDaemon consumes the fast-exit session's egress; production `Drain` is the mechanism that advances and exposes that completion. Without an Attach subscription this path emits `SessionLifecycle`, while `ProcessExit` is attached-subscription client egress and is therefore intentionally not an acceptance requirement here. Adding Attach/Detach solely to obtain that second representation would broaden the test into unrelated subscription ownership and cleanup. `ReadScreen` is also not used because it is explicitly allowed to fail after fast exit. Observing exact drained lifecycle plus retained `exited` proves the PTY-spawned shell has completed. The unchanged fixture writes the relative path `live-managed.txt`, so reading the exact expected content from `<returned-worktree>/live-managed.txt` after that lifecycle boundary proves the shell ran in the managed cwd. The existing `LOCAL_RUNTIME_DAEMON_READINESS_BUDGET` is a hang backstop, not a readiness threshold; no timeout is increased and no blind file retry remains.

## Scope

- Add the smallest test-local condition-driven helper needed to issue production `Drain` only for the returned session UUID until its exact `SessionLifecycle(exited)`, then request `ListSessions` and require retained lifecycle `exited` within the existing runtime readiness budget, failing immediately on lifecycle `failed`, unexpected lifecycle, disappearance, or daemon/transport error.
...
- Replace this test's three raw daemon `Child` owners (first, competing, restarted) with `PanicSafeCliDaemon`; use its consuming `shutdown()` on normal transitions so `Drop` owns every assertion/readiness unwind. Extend the existing test-local guard's panic path to enumerate sessions in its isolated data directory, issue production `ShutdownSession` for every non-exited or stale/failed row, record pre/post responses without replacing the original panic, and only then run its existing Hub shutdown/reap path.
...
- Production `Drain` consumes the exact session's egress and exposes non-attached `SessionLifecycle(exited)`; the canonical `exited` lifecycle is published only after the PTY child has completed, so the fixture's preceding relative file write has completed before the assertion reads it.
...
- `PanicSafeCliDaemon::shutdown()` uses the validated shutdown classifier on normal transitions. Its panic-time `Drop` first performs bounded, best-effort exact-session shutdown through the production request path, then performs best-effort Hub cleanup and diagnostics; neither cleanup stage may replace the inducing panic. Panic cleanup is proven by exact PID, zombie, metadata, and socket census, not by treating its shutdown-disconnect handling as validated success.
...
- The session may be `running` or `stopping` on early `ListSessions` observations; those reachable transitional states (and a not-yet-projected row after a successful spawn result) remain in the condition-driven wait. This projection cannot emit `starting`, although the richer drained lifecycle event vocabulary can. `failed`, an unexpected lifecycle, disappearance after first observation, or daemon/transport error fails with every drained event, the full last responses, elapsed budget, and whether the target row was ever observed. On the ListSessions path `failed` specifically identifies the stale-registry projection (`RegistrySessionState::Stale`); `DaemonSession` does not carry the underlying failure reason, so diagnostics name that mapping rather than misattributing it to command failure.
...
1. Add a narrow helper beside the managed Git test that takes data dir and the returned session UUID. Repeatedly issue production `DaemonRequest::Drain` only for that UUID until its exact `SessionLifecycle(exited)`, then issue `DaemonRequest::ListSessions`, locate only that session, and return when it is retained at lifecycle `exited`; retain reachable transitions or a not-yet-projected successful spawn, fail on stale-registry `failed`, unexpected lifecycle, post-observation disappearance, or request error, and stop at `LOCAL_RUNTIME_DAEMON_READINESS_BUDGET` with every drained event, full last responses, and whether the target row was ever observed in diagnostics.
...
3. Replace `first_child`, `competing_child`, and `second_child` with `PanicSafeCliDaemon` instances as soon as each daemon starts. Replace normal raw-child shutdown calls and redundant output checks with the guard's validated consuming `shutdown()`. On panic, have the guard list only its isolated data-dir sessions, issue production `ShutdownSession` for every row not conclusively exited (including stale/failed rows), record every response and post-cleanup listing without panicking, and then always continue its existing Hub reap. Keep explicit session shutdown before final daemon shutdown. Do not touch unrelated raw children.
...
- **Wrong session or nonterminal state creates false readiness:** drain only the exact returned session UUID, require its exact `SessionLifecycle(exited)`, and then require canonical retained lifecycle `exited`; `running`/`stopping` are not success.
...
- **Panic cleanup hides the original assertion:** wrap exact-session cleanup defensively and keep both session and Hub `Drop` cleanup best-effort and diagnostic; the inducing panic remains primary even if cleanup itself fails.
...
2. **Focused production-path regression**
   ...
   - The passing log must positively show the returned session's exact drained `SessionLifecycle(exited)` followed by retained `exited`, marker equality at the returned worktree path, competing-Hub rejection, restart reconciliation, reused worktree, distinct second session, explicit session shutdowns, and clean daemon shutdowns.
...
6. **Runtime wiring proof**
   - Review the final test trace to show that success depends on `PluginMcpCallTool` returning a real session UUID, production `Drain` observing that exact session's non-attached `SessionLifecycle(exited)`, production `ListSessions` retaining that exact session at `exited`, and only then the unchanged relative-path marker assertion succeeding. A unit-only helper or direct filesystem wait is insufficient; attached-subscription `ProcessExit` is intentionally not required on this path.
