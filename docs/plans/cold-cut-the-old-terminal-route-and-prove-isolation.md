...
| Review-return Web dashboard and keep-alive | `question_1788477409_664609` option C | One botster-web ticket covers both remaining live failures: display Hub-authored `lifecycle_class=ended` rows through an explicit ended-session path, and repair caller-owned shared-session keep-alive so `botster-web-production-alt-exited` appears. Do not relabel ended as current. Do not infer lifecycle from `registry_state`. Do not waive either lane. Do not add another TUI ticket. Keep the WebRTC WouldBlock wait repair in this Hub run with disabled-seam ablation. This integration ticket depends on `ticket_1788477497_716720`. After that Web merge, rerun durable dashboard, north-star shared session, TUI `ghostty-shared`, and the complete matrix once. |
| North-star producer alt-exit | `question_1788477409_664609` | Keep the producer repair in this Hub integration run. `script/prove-north-star-shared-session` must handle `botster-web-production-alt-exit`, leave the alternate screen (`ESC[?1049l`), echo `botster-web-production-alt-exited`, and return to the primary screen before TUI attach. Do not create another Hub ticket. After `ticket_1788477497_716720` merges, run the complete north-star lane. Keep the alternate-screen and `ghostty-shared` assertions unchanged. |
| Final-matrix Ghostty and north-star | Fable confirmation of `ticket_1788537020_814817` | Core `ticket_1788537020_814817` is required. After that Core merge and the Hub pin, the final matrix must run `script/test-live-hub ghostty` and `script/prove-north-star-shared-session`. Both must pass before Hub merge. Do not skip those arms. Do not transfer another Core or Hub close-evidence defect to TUI. Durable TUI pin-roll `ticket_1788460430_647093` remains the consumer Cargo pin after this merge. |
...
- TUI (scratch worktree of `botster-tui` at `origin/main`): roll `botster-hub-client` and `botster-hub-test-support` to the Hub candidate commit and every Core pin to the Hub-locked Core revision in `crates/botster-tui/Cargo.toml`, `Cargo.lock`, and the `app.rs` live-lane defaults, uncommitted, for proof only. Run `script/test-live-hub ghostty` with `BOTSTER_HUB_BIN_REV=<candidate>` and `BOTSTER_SESSION_WORKER_BIN_REV` equal to the Hub-locked Core revision. Record the exact TUI diff in the report. The durable TUI roll is the consumer ticket in section 7. The final matrix after `ticket_1788537020_814817` must run this arm. Do not skip it.
...
17. After `ticket_1788537020_814817` merges and Hub pins it, the final matrix unskips `script/test-live-hub ghostty` and `script/prove-north-star-shared-session`. Both must pass before Hub merge. Do not transfer another Core or Hub close-evidence defect to TUI.
