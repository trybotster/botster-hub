...
3. **Every queued Response stays in the write loop.** After enqueueing a Response, keep calling `flush_unix_mux_writes` until that frame is Written or `DAEMON_CLIENT_WRITE_TIMEOUT` elapses. Do not read another request while that Response remains pending. Do not start `SubscribeEntities` while `MuxWriteState` has data, the Unix mux has unsent frames, or any terminal route remains bound. Reject that transition with an `OperatorError` and keep flushing the mux. Do not `close_all()` as the reject. `close_after` still waits the same way, then sets `ConnectionTerminalReason::NormalClose` and returns. Timeout uses the existing write-failure path. This replaces the current "write_all then immediately return" sequence without unbounded `block_on`.
