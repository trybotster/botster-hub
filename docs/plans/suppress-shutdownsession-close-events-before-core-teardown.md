...
1. Gates: `cargo fmt --all -- --check`; `cargo clippy --workspace --all-targets --locked -- -D warnings`; then `cargo build --locked -p botster-core-daemon --bin botster-session-worker` followed by `./test.sh --locked` (a clean worktree needs the pinned session-worker binary before the full suite; Plan Review finding `finding_1787144924_108808`). Colon-free worktree confirmed; tracked `.gitignore` intact at HEAD.
...
3. Live Unix Core-error production path (primary proof; Plan Review finding `finding_1787144924_300861`): extend `external_hub_shutdown_session_failure_keeps_daemon_and_sibling_usable` (`tests/hub_daemon_lifecycle/sessions.rs:3510`), which already drives the compound construction — drain injection makes classification fall through as Active, then SIGKILL makes the live Core `Shutdown` fail — through the real daemon binary. Add to it: attach the victim session on the unix-adapter connection and record its Core-issued generation before the failed shutdown; after the failed shutdown assert (a) the exact typed OperatorError body, (b) the victim adapter is closed under suppression, (c) zero `TerminalSubscriptionClosed` frames for the victim `(session, subscription, generation)` while the connection keeps reading, and (d) sibling terminal envelopes keep flowing on the same connection.
3a. Unit, handler decomposition (focused support only, not the production proof): suppression keys installed before the Core call; `close_adapters_for_session` host-close under suppression produces no event on a subsequent `Running` sweep; `recover_from_exact_classify` returns the exact typed results per classification (Cleanup/Missing/Stopping/Active+already-gone/Active+error).
