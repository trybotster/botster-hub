...
Run: run_1787076383_328340. Plan rev 1.
...
5. The fixture test itself passes: after Hub exit the zombie is reaped by the system, so `prove_owned_absence` (which already has an explicit zombie check at `harness.rs:831-838`) succeeds.
...
3. **Classifier fix.** In `tests/hub_daemon_lifecycle/harness.rs`, add one helper, e.g. `fn recovery_worker_is_live(pid: u32) -> bool { process_exists(pid) && !process_snapshot(pid).is_some_and(|s| s.stat.contains('Z')) }`, reusing the exact zombie idiom from `prove_owned_absence` (`stat.contains('Z')`). Use it in:
...
- No changes to `prove_owned_absence` (its zombie check already fails closed after Hub exit).
...
- Assumption: `ps -o stat=` reports `Z` for zombies on Darwin 25.5.0 (the harness already relies on this in `prove_owned_absence`).
...
- **Over-classification**: a stat string that contains `Z` for a non-zombie would skip the taint. Darwin state letters make this a non-collision; the same idiom already guards `prove_owned_absence`.
...
2. Targeted: `cargo test --test hub_daemon_lifecycle -- harness_isolation` green, including unmodified `dead_command_with_live_unverified_recovery_worker_taints`, `dead_command_and_dead_recovery_worker_do_not_taint`, `dead_command_without_recovery_identity_taints_and_does_not_signal`.
3. Targeted: `cargo test --test hub_daemon_lifecycle external_hub_shutdown_session_failure_keeps_daemon_and_sibling_usable -- --nocapture` passes with **no** `identity capture incomplete` stderr line.
