...
3c. Targeted: `./test.sh --locked --test hub_daemon_lifecycle_test injected_taint -- --exact` — `injected_taint_cannot_race_an_unguarded_real_daemon_start` waits on a start-boundary hook fired immediately before `daemon_test_guard` acquisition (parent lock and taint stay set until that hook). `injected_taint_race_fails_when_start_guard_is_bypassed` is the narrow guard-bypass ablation and must fail the child start while taint is still set.
