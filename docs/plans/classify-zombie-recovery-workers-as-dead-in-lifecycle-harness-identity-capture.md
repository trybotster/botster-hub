...
3c. Targeted: `./test.sh --locked --test hub_daemon_lifecycle_test -- injected_taint sibling_real_daemon` — the start-boundary hook is identity-scoped (thread-local token set before `start_cli_daemon`). `injected_taint_cannot_race_an_unguarded_real_daemon_start` waits for the intended token. `injected_taint_race_fails_when_start_guard_is_bypassed` is the lock-bypass ablation. `sibling_real_daemon_start_cannot_satisfy_intended_boundary_hook` proves a foreign first-acquire cannot complete the intended receiver.
