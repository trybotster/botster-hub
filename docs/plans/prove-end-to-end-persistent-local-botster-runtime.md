...
- Repo context inspected: `README.md`, `Cargo.toml`, `test.sh`, `src/main.rs`, `src/runtime.rs`, `src/daemon.rs`, `src/client_api.rs`, `tests/hub_daemon_lifecycle_test.rs`, `tests/hub_client_api_test.rs`, `tests/hub_local_runtime_test.rs`, and prior `docs/plans/*`.
- Current repo state observed:
  ...
  - `tests/hub_local_runtime_test.rs` already proves package lifecycle, hub-state reload, plugin lifecycle observation/invocation, session spawn/attach/input/drain/shutdown in one in-process production runtime flow.
  - `README.md` already documents daemon-backed CLI commands, restart behavior, production runtime-ready scope, and feature-parity gaps.
...
- Add a concise readiness note in repo docs, either as a new `docs/adr`/`docs/plans` handoff or a README section, stating whether this stack is ready for local production runtime and the exact remaining gaps before replacing the monolith locally.
...
- Assumption: "Persistent local Botster runtime" means durable local production runtime readiness for the production-shaped local stack: hub as host profile/control plane, core daemon as multiplexer/supervisor/router, and session workers as durable PTY owners.
...
- Unknown: Whether a README-only readiness note is enough. If the implementation produces nuanced production runtime readiness/gap analysis, prefer a small `docs/adr` or `docs/plans` readiness document linked from README.
...
- `tests/hub_local_runtime_test.rs`
  - Keep package/plugin production runtime proof green.
  - Extend only if the final proof should include package lifecycle in the same test rather than relying on a daemon lifecycle test plus this production runtime test.
- `README.md`
  - Ensure the documented fresh-checkout commands exactly match what was verified.
  - Add or link the readiness note and remaining local production runtime gaps.
  - Make hub restart vs core daemon restart semantics concrete.
- Optional new readiness document under `docs/adr/` or `docs/plans/`
  - Summarize production runtime readiness and remaining gaps before replacing the monolith.
...
- Underwired proof risk: tests that call `HubRuntime` directly without touching the documented user path are useful but not enough. At least one proof must name and exercise the production entry point a local production runtime user follows.
...
- A final proof command, preferably one of:
  ...
  - or `./test.sh --test hub_local_runtime_test local_runtime_runs_daemon_package_lifecycle_session_and_clean_shutdown` plus a documented daemon-restart test matrix if one test would duplicate too much harness code.
...
- Readiness note states whether the new stack is ready for local production runtime and lists remaining gaps before broader feature parity.
