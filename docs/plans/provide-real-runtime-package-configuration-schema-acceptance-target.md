# Provide Real Local runtime Package Configuration Schema Acceptance Target
...
- Ticket: provide hub/local runtime support for a real local first-party package that exposes an authoritative package configuration schema and deterministic validation behavior suitable for botster-web acceptance.
...
- Repo context inspected: `src/main.rs`, `src/packages.rs`, `src/daemon_transport.rs`, `crates/botster-hub-client/src/lib.rs`, `tests/hub_daemon_lifecycle_test.rs`, `README.md`, `docs/client-protocol.md`, `examples/synthetic-plugin/botster-package.json`, and prior local runtime/package-configuration plans.
...
- Add a real first-party local runtime package configuration acceptance target that downstream botster-web acceptance can use without local-only browser fixtures.
- Prefer the smallest path: extend the existing local runtime acceptance fixture package set with a checked-in or test-generated first-party-style package whose manifest declares a deterministic `configuration` schema.
- Ensure the package is installable/enabled through the `botster-hub local runtime bootstrap` path or the same daemon-owned local package path used by local runtime tests.
...
- Update README or client protocol docs only if a new named local runtime package/path/acceptance command becomes part of the supported operator contract.
...
- No broad local runtime refactor, new package lifecycle abstraction, Project Pipelines workflow change, or UI workbench work.
- No secret raw-value round trip. Existing redaction semantics must remain intact.

## Assumptions And Unknowns

- Assumption: "real local first-party package" means a package participating in the hub/local runtime first-party package path, not the existing ad hoc `configurable.plugin` temporary test package.
...
- Unknown: whether local runtime bootstrap should enable this package by default for daily operators. If adding it to default local runtime would force required config before enablement, ask a human rather than silently weakening validation.

## Botster Layers Touched

- Rust hub CLI/local runtime path: the removed bootstrap command package installation and printed operator contract if the package becomes a named local runtime input.
...
- `tests/hub_daemon_lifecycle_test.rs`: add or extend a focused real-daemon acceptance test proving the local runtime package configuration target through install/enable/list/set/reject/list/restart or persisted refresh.
...
- `src/main.rs`: only if the removed bootstrap command must accept or install the new package target as an official first-party local runtime input.
- `README.md` and/or `docs/client-protocol.md`: only if a new supported local runtime acceptance package/command is introduced.
...
- The proof must use the running daemon owner path, not direct registry mutation:
  - install/enable through `DaemonRequest::InstallPackageLocalPath`, `EnablePackage`, or the removed bootstrap command;
...
- Fixture risk: extending the existing synthetic `configurable.plugin` test would satisfy mechanics but not the "local runtime first-party" acceptance target. The package name/path must make the first-party local runtime intent explicit.
- Enablement risk: a required-field schema can block package enablement during local runtime bootstrap. If the acceptance package must be enabled by default, it should either have safe defaults or be installed-but-disabled intentionally and documented.
...
- Path/PII risk: local runtime output and plan/report artifacts should not expose local home paths.

## Acceptance Checks And Tests

- Focused command:
  - `./test.sh --test hub_daemon_lifecycle_test <new_or_updated_local_runtime_package_configuration_acceptance_test> -- --test-threads=1`
...
- Review should reject code-only evidence that does not prove the running daemon/local runtime path.
...
None found. The plan follows the loaded Botster constraints: hub owns package/runtime policy, public clients consume authoritative Rust daemon DTOs, local runtime remains a local first-party package path, workflow policy stays out of core, and test evidence uses the repo wrapper rather than direct `cargo test`.

## Vault Gaps Worth Capturing

- Capture a durable note if this ticket establishes a named convention for "first-party configurable package acceptance target" in local runtime.
- Capture a note if implementation discovers that default local runtime enablement and required package configuration schemas need a standing rule.
