...
Revision: v3.2, addressing Verify `review_1787005607_892358`, advisor answer `question_1787005268_112714`, Review `review_1787003842_760636`, Review `review_1787004220_658926`, and durable answer `question_1787003911_553236`. The v3 product repair is unchanged. The snapshot-assembly integration oracle no longer asserts that the first Snapshot frame contains 24 items. Subscribe catch-up is sliced, so the test drains Snapshot and later Upsert/Patch frames until all 24 load-session identities appear. Acceptance check 4 is no longer a five-run unfiltered lifecycle suite. This ticket's Implement and Review gate is focused proof plus known-baseline exact-bytes evidence. Final integration remains the only strict zero-failure convergence gate. The Risks mitigation and other extraction-gate sentences must use that same focused contract.
...
6. Keep every existing deterministic functional assertion except the first-snapshot count: the `Spawn` reply succeeds, unsubscribes succeed, and the daemon shuts down cleanly. For the snapshot-assembly test, drain Snapshot and later Upsert/Patch frames until all 24 `assemble-session-*` identities appear. Do not assert that the first Snapshot frame contains 24 items. Subscribe catch-up uses capped `JournalPull` and `ProjectionApply` slices (16 changes), so a complete first Snapshot can still omit later load-session rows. A drain deadline is hang protection only, not a latency oracle. Authorized by Verify `review_1787005607_892358` and advisor answer `question_1787005268_112714`.
...
- First-snapshot count is not a deterministic catch-up oracle. Subscribe applies at most 16 journal changes and 16 projection applies before the first Snapshot can flush. A 24-session fixture can therefore observe a complete Snapshot with fewer than 24 rows. Mitigation: drain later Upsert/Patch frames until all 24 load-session identities appear. Do not change production slice budgets.
