...
Revision: v4.2. This revision amends v4.1 after Review `review_1787026690_762581` (`changes_required`). v4.1 remains the accepted hold-path contract. v4.2 retires pending Spawn acknowledgements after the projection observes them, recovers omitted journal rows from Core's retained floor instead of sequence 0, and removes the unbounded Core `list()` read from subscriber delivery. The 24-identity invariant stands. Production budgets stay unchanged.
...
- `src/daemon_maintenance.rs` — watermark field, confirming-empty-pull flag, capture, clear-on-resync and clear-on-mutation, pending Spawn acknowledgements with retire-after-project, retained-floor recover, completion predicate, apply-to-delivery prefer, unit tests. No subscriber-delivery `list()`.
...
- `docs/plans/fix-flaky-ready-spawn-wall-clock-budget-under-ambient-load.md` (this plan, v4.2) and `docs/reports/fix-flaky-ready-spawn-wall-clock-budget-under-ambient-load-implement.md` (updated implement report).
