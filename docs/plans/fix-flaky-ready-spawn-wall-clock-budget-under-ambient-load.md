...
2. Define one projection-completion predicate on `MaintenanceState` (for example `projection_caught_up()`): baseline sealed (`projection.baseline_complete`), no gap, no in-progress baseline recovery, `pending_changes` empty, stored watermark present with matching `source_id`, and `projection.cursor.sequence >= watermark.sequence`.
3. Gate first-snapshot completion on that predicate. `continue_session_snapshot_assembly` holds (returns a zero-progress page with `more: true`, as the current `!baseline_complete || gap` branch does) until the predicate passes. The registration path passes the predicate result (or the needed state) into assembly. The held subscription keeps `needs_delivery = true`, so the owner loop keeps re-waking, pulls the journal to the watermark, applies pending changes, and then assembles and sends the one complete `Snapshot` frame.
...
- Unknown: the exact historical cause of the 8-of-24 freeze in the subscribe-only experiment. The plan's wake-chain reading (fact 3) says catch-up self-drives; the existing-subscriber proof (scope 7) and the first-frame oracle (scope 6) will surface any residual wake gap as a deterministic failure, and Implement must then diagnose the wake chain rather than widen budgets or timeouts.
...
- `src/daemon_maintenance.rs` — watermark field, capture, clear-on-resync, completion predicate, unit tests.
- `src/daemon_entity_subscriptions.rs` — assembly hold condition, registration path plumbing, unit tests (including updating `first_session_snapshot_is_complete_and_assembled_in_pages` and neighbors to the new gate signature).
- `tests/hub_daemon_lifecycle/sessions.rs` — rewritten snapshot-assembly test, existing-subscriber proof, removal of catch-up machinery.
- `src/daemon_transport.rs` — expected untouched; only touched if a proven wake-chain gap requires it.
- `docs/plans/fix-flaky-ready-spawn-wall-clock-budget-under-ambient-load.md` (this plan, v4) and `docs/reports/fix-flaky-ready-spawn-wall-clock-budget-under-ambient-load-implement.md` (updated implement report).
...
7. Final smoke: one exclusive full `./test.sh --locked --test hub_daemon_lifecycle_test` run, requested through an orchestrator slot (do not start an unfiltered suite without one). Zero failures required except an exactly-evidenced known-baseline failure owned by `ticket_1786977409_499180`.
8. Production-path proof statement in the implement report: the gate sits in `continue_session_snapshot_assembly`, which is the only session first-snapshot producer, reached from both `register_entity_subscription` and the `SubscriberDelivery` slice; no scaffold-only code.
