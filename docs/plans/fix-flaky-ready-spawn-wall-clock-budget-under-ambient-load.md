...
Revision: v3.3, addressing Review `review_1787006443_517771`, Verify `review_1787005607_892358`, advisor answer `question_1787005268_112714`, Review `review_1787003842_760636`, Review `review_1787004220_658926`, and durable answer `question_1787003911_553236`. The v3 product repair is unchanged. The snapshot-assembly oracle no longer waits for all 24 load-session identities. Subscribe does not run Observe, Observe publishes eight live rows per slice, and extra Subscribe turns starve Observe. Review authorized revising the oracle without a load-sensitive deadline. The test keeps the 24-session fixture, asserts the ready Spawn, requires one live subscription frame, and fails immediately on `DaemonEntityFrame::Error`. Acceptance check 4 is no longer a five-run unfiltered lifecycle suite. This ticket's Implement and Review gate is focused proof plus known-baseline exact-bytes evidence. Final integration remains the only strict zero-failure convergence gate. The Risks mitigation and other extraction-gate sentences must use that same focused contract.
...
6. Keep the ready `Spawn` reply, unsubscribe, and clean shutdown. Do not assert first-snapshot count or full 24-identity catch-up. Subscribe does not run Observe, so a 24-row identity drain is a loop-tick window. Extra Subscribe turns starve Observe and can freeze the projected set at one observe slice (8 rows). The snapshot-assembly test requires one live subscription frame and fails immediately on `DaemonEntityFrame::Error`. Authorized by Review `review_1787006443_517771` option to revise the oracle without a load-sensitive deadline.
...
- First-snapshot count and full 24-identity drains are not deterministic catch-up oracles. Subscribe does not run Observe. Observe publishes eight live rows per slice. A 30-second quiet drain failed at 17 of 24 identities (`review_1787006443_517771`). Extra Subscribe turns then froze the set at 8 rows by starving Observe. Mitigation: keep the 24-session load fixture, assert ready Spawn plus one live frame, and fail immediately on `DaemonEntityFrame::Error`. Do not change production slice budgets.
