...
Revision: v3, addressing Plan Review `review_1786945973_864427` and `review_1786947010_249302` (both changes_required). The v1 load-window liveness oracle is withdrawn. The v2 biased-select oracle is withdrawn as timing-dependent and probabilistic. The ordering proof is one deterministic decision-level unit oracle on the owner loop's busy-path classification, with a negative control that ablates that exact scheduling decision.
