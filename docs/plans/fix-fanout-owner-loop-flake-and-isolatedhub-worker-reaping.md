...
### Visit 10 response to `review_1787902370_651553`

Two major findings, both verified and correct.

**`finding_1787902370_660673`: the typed quiescence failure fell through the consuming `shutdown` and `Drop` path.** Confirmed against the code. `IsolatedHub::shutdown(mut self)` **consumes** the guard, so the typed quiescence error is produced while `self` is being dropped. The current `Drop` (`crates/botster-hub-test-support/src/isolated_hub.rs:315`) calls `shutdown_inner()` again and, on failure, calls `cleanup_child`, which sends group signals **without a confirmed complete descendant snapshot** — recreating precisely the stale-identity leak visit 9 removed. My claim that resources stay available "for a later attempt" was also empty, because no caller retains `self`. And `Drop` cannot return a typed error at all.

This is the same failure mode as my previous errors, one layer out: I specified a failure *semantic* without tracing what the language and the object lifecycle actually do with it.

Resolution:

1. **Explicit lifecycle state.** `IsolatedHub` carries `Completed | Pending | QuiescenceUnconfirmed { pgid, data_dir }`.
2. **`Drop` is gated by that state.** `Completed` does nothing. `Pending` runs the normal safe path. `QuiescenceUnconfirmed` must never re-enter `shutdown_inner`, never call `cleanup_child`, and never signal a remembered set.
3. **At most one fresh-sample retry.** In `QuiescenceUnconfirmed`, `Drop` may retry the safe sequence once, taking a **new** freeze, stop confirmation, and snapshot under the residual budget. It never reuses an earlier set.
4. **Ownership handoff instead of a dangling claim.** If the retry also fails, the guard resumes the group and the instance records the unresolved `(pgid, data_dir)` in a process-global taint cell owned by `botster-hub-test-support`, and `IsolatedHubBuilder` refuses to start while it is set. That makes a *reachable* mechanism own the still-running processes, which is what the fix asked for.
5. **Both paths are tested.** Acceptance 14(e) covers explicit `shutdown(self)`, 14(f) covers direct `Drop`, 14(g) proves the taint blocks the next start, and 14(h) proves the retry takes a fresh sample.

**Worth the owner's attention:** the shipped harness taint (`HARNESS_TAINT`) lives in `tests/hub_daemon_lifecycle/harness.rs`, which a library crate cannot reach, so this adds a second, scoped taint cell inside `botster-hub-test-support`. That is the same crate-boundary constraint that already forces a second worker census, and it is recorded in `ownership_identity`. I judged it in scope as fixture hygiene rather than the general process supervision the owner excluded in `question_1787900642_912254`, and the reviewer's own suggested fix names a "durable recovery or taint mechanism" as acceptable. Flagging it rather than blocking, since both the reviewer and the existing repository convention sanction it.

**`finding_1787902370_764228`: a review requirement still demanded the deleted live-snapshot fallback.** Confirmed and corrected. That requirement now states the single-sample rule and the no-stale-set rule, and the consuming-`shutdown` and `Drop` safety requirement is added to the same review section.
...
| `ownership_identity` | **Restored on visit 8; the visit-7 edit dropped this row, see `finding_1787901105_233367`.** Owner identity is `(argv0 basename == "botster-session-worker", pgid == this instance's Hub-child pid)`, plus the parent/child descendant closure of each such worker, computed against the **frozen and stop-confirmed** group on the timeout path and while the workers are alive on the normal path. The Hub-child pid is captured into the `IsolatedHub` struct at spawn, so it survives `self.child = None` on the `Drop` path and stays available after a phase timeout. This identity needs no argv inspection of the data directory, no path canonicalization, and no worktree heuristic. To close PID recycling after the Hub child exits, the owned-set snapshot bounds what may be signalled: the reap never signals a PID that is neither in the snapshot nor in the captured descendant closure. The freeze, the stopped-state census, and the kill all use the same verified owned pgid, so none of them can reach anything outside the owned set. Each IsolatedHub has a distinct Hub-child pid for its lifetime. **Unresolved ownership is handed off, not dropped:** when quiescence is never confirmed, the instance records `(pgid, data_dir)` into a process-global taint cell owned by the `botster-hub-test-support` crate, and `IsolatedHubBuilder` refuses to start a new instance while that cell is set. This mirrors the existing harness-taint convention in [[harness identity capture errors taint later daemon starts]] and [[real daemon start boundaries serialize against process global taint]]. **A new cell is required because the shipped `HARNESS_TAINT` static lives in `tests/hub_daemon_lifecycle/harness.rs`, which a library crate cannot reach** — the same crate-boundary constraint that already forces a second worker census. |
| `sibling_fail_closed_policy` | **Ultimate-failure policy rewritten on visit 9 for `finding_1787901743_665821`.** On a successful teardown, siblings keep working; nothing outside the owned set is signalled, frozen, or stopped. Ultimate failure now has two distinct shapes, and **neither claims success**. (a) *Quiescence not confirmed within the sub-budget*: the freeze guard resumes the owned group with `killpg(hub_pgid, SIGCONT)`, the instance records the lifecycle state `QuiescenceUnconfirmed { pgid, data_dir }`, and teardown returns a typed `IsolatedHubError`. The Hub and its workers are left **running** — not killed, not stopped — precisely so their parent links survive. **Because `IsolatedHub::shutdown(mut self)` consumes the guard, `self` is dropped before the caller sees that error, so the state must survive into `Drop` and `Drop` must be safe.** In `QuiescenceUnconfirmed`, `Drop` must **not** call `shutdown_inner` again, must **not** call `cleanup_child`, and must **not** signal any remembered set. It may make exactly one further attempt at the safe sequence — a **fresh** freeze, stop confirmation, and snapshot under the residual budget — and if that also fails it resumes the group and hands the unresolved `(pgid, data_dir)` to the harness taint described in `ownership_identity`, so the next `IsolatedHubBuilder` start refuses rather than racing an unowned Hub. The plan does **not** claim zero owned descendants in this case, and the fault tests assert the documented typed failure and the taint rather than an absence they cannot prove. (b) *Reap budget expires with a non-empty census after a confirmed-stopped kill*: the reap returns without escalating and without sacrificing any sibling. In both shapes the blast radius stays bounded to the owned set and nothing is left stopped. `SIGSTOP`, `SIGCONT`, and `killpg` are sent only to a group this instance owns: the Hub-child group, or a captured PTY descendant's own leader group. None is ever sent to a non-leader PID nor to this process's own group, so the harness cannot stop or kill the `cargo test` runner. The plan explicitly rejects `6012eca`'s unconditional `killpg` on a non-leader PID for the same reason. |
...
9. **The consuming `shutdown(self)` drops the guard before the caller sees the error.** `IsolatedHub::shutdown(mut self)` takes ownership, so a typed quiescence error is produced while `self` is being destroyed. The old `Drop` then called `shutdown_inner` again and could reach `cleanup_child`, which signals the group without a confirmed snapshot and recreates the stale-identity leak. The claim that resources stay available "for a later attempt" was also empty, because no caller retains `self`. Mitigation: an explicit lifecycle state that makes `Drop` safe in `QuiescenceUnconfirmed`, one fresh-sample retry at most, and a taint handoff so a reachable mechanism owns anything still running. Acceptance 14(e) through 14(h) are the controls.
10. **Core changes the worker's process group.** Assumption 5 is a cross-repository behavioral dependency on the pinned Core revision. Mitigation: Implement re-verifies it against the lockfile revision and records the check; the named fallback is a pre-exit descendant snapshot.
11. **The bounded wait reintroduces a pipe deadlock.** `wait_with_output` exists partly to drain the child's piped stdout and stderr; a bare `try_wait` poll would let a chatty child block on a full pipe buffer. Mitigation, corrected on visit 4: `take()` the `ChildStdout` and `ChildStderr` and drain each on its own thread while the teardown owner keeps the `Child` and polls `try_wait`. Draining continues and the `Child` stays available for `cleanup_child`. Acceptance check 11(c) is the control.
12. **A leaked drain thread after a phase timeout.** A timed-out phase can leave a drain thread blocked reading a pipe that never closes. Mitigation: the timeout path signals and reaps the child through the retained `Child` **before** it joins, so each pipe closes at process exit and both drain threads return. Acceptance check 11(d) requires proof that every drain thread exits after a timeout, and Implement must confirm no drain thread outlives the teardown.
13. **Reaping masks a real Hub leak.** A test-support reap can hide a genuine failure to stop workers. Mitigation: [[hub shutdown preserves durable session workers]] records that Hub stop intentionally preserves durable workers, so the reap is fixture hygiene rather than a cover for a Hub defect. Review must confirm the reap is not called in place of an assertion that a production path already owns.
14. **`Drop` panics.** A panic in `Drop` during unwinding aborts. Mitigation: the reap swallows every error and the existing `thread::panicking()` guards stay.
15. **The gate stays flaky for a third cause.** Two causes are now known. A third may exist. Mitigation: the gate must pass at default workspace concurrency without a waiver, and isolated reruns do not count as proof.
16. **Strict Clippy hides later diagnostics.** Per [[strict clippy can hide later crate diagnostics behind the first compile failure]], one green run after a repair is not proof. Mitigation: rerun the full workspace Clippy after every repair.
17. **Wrong toolchain in the pipeline shell.** Per [[botster Hub pipeline shells can override RUSTUP TOOLCHAIN below the CI pin]], a bare strict Clippy can exit 0 on unfixed code. Mitigation: `RUSTUP_TOOLCHAIN=1.97.0` on every gate command, with `rustc --version` recorded from that shell.
...
14. **Freeze fault-injection proofs, added on visit 8 for `finding_1787901105_392038`.** (a) *Delayed stop confirmation*: a worker that reaches the stopped state only after a delay must still be captured, proving the path waits for the stopped-state census rather than trusting the `killpg` return; with the census removed this proof must fail. (b) *Quiescence not confirmed*, rewritten on visit 9: with stop confirmation forced to fail, teardown must **not** kill on a stale set. It must resume the owned group with `SIGCONT`, return the documented typed `IsolatedHubError`, and leave every owned process **running** rather than stopped. The assertion is on the typed failure and on the absence of stopped processes; it must **not** claim zero owned descendants, because this path deliberately does not establish that. A companion arm shows why: with a worker and a separate-process-group PTY created during the Hub-wait phase, the stale early snapshot does not contain that PTY, so a stale-set kill would strand it, and the test asserts that this kill does not happen. (c) *Freeze-guard resume*: on a path where termination cannot proceed, the guard must send `SIGCONT`, including when a panic unwinds through `Drop`, and the test must observe every owned process either gone or running, never stopped. (d) *Idempotence under freeze*: a second teardown on an already-clean instance must signal nothing and must leave no stopped process. (e) **Consuming-shutdown safety, added on visit 10 for `finding_1787902370_660673`**: call `IsolatedHub::shutdown(self)` with quiescence forced to fail. The typed error must reach the caller, and the `Drop` that runs as `self` is consumed must **not** call `cleanup_child` and must **not** signal any remembered set. The test asserts no stale-set kill, no stopped process, and that the taint cell now names the unresolved `(pgid, data_dir)`. (f) **Direct-Drop safety**: the same scenario without calling `shutdown`, letting the guard fall out of scope. `Drop` cannot return an error, so the test asserts the same three properties through the taint cell and a live `ps` census. (g) **Taint blocks the next start**: with the taint set, `IsolatedHubBuilder` must refuse to start a new instance and must not spawn a daemon or a worker, mirroring `taint_latch_refuses_next_daemon_start_without_spawning`. (h) **Drop retry takes a fresh sample**: when `Drop` retries the safe sequence, it must re-freeze and re-confirm rather than reuse the earlier set; with the retry forced to reuse a remembered set, the test must fail.
...
- **Added on visit 8:** Review confirms the path never treats a returning `killpg(SIGSTOP)` as proof of quiescence, that a bounded stopped-state census gates the frozen snapshot, that a freeze guard makes `SIGCONT` unconditional on every non-terminating return path including a panic unwinding through `Drop`, that the confirmed-stopped sample is reused as the sole completeness set with no second fallible census and no live-snapshot fallback, that unconfirmed quiescence resumes the group and returns a typed failure instead of killing, and that no normative section still prescribes the superseded pre-signal snapshot, live-snapshot fallback, or `cleanup_child` timeout flow.
- **Added on visit 10:** Review confirms the consuming `shutdown(self)` and the direct `Drop` path are both safe after unconfirmed quiescence: `Drop` must never call `cleanup_child` or signal on a remembered set in that state, any `Drop` retry must take a fresh freeze, confirm, and snapshot, and a reachable mechanism must own any still-running owned processes. Review checks both paths have tests.
...
13. **A consuming method's error is delivered while the object is dying.** A `fn shutdown(mut self)` that returns `Err` runs `Drop` before the caller reads it, so any failure semantic that promises "left in state X for a later attempt" is empty unless an explicit lifecycle state makes `Drop` honor it and something reachable takes ownership. Candidate note: "a consuming teardown needs a Drop-visible state, not a promise in the error".
