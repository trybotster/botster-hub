...
6. **Reparenting destroys descendant links on the timeout path, and a bare pre-signal snapshot does not close it.** `cleanup_child` signals the Hub process group, which contains the session workers, and a worker can create a new-pgid PTY between any snapshot and that signal. The worker then dies, the PTY is reparented to init, and neither the captured set nor a live parent/child census can find it. Mitigation: freeze the owned group with `SIGSTOP` first, snapshot against the frozen group, then `SIGKILL` and reap the fixed set. Acceptance check 11(b) plus the new fork-and-reparent race proof are the controls, and the race proof must fail without the freeze.
