...
| `production_path_proof` | **Rewritten on visit 5 for `finding_1787899600_156636`; the previous text named `child.wait_with_output()`, which the corrected mechanism removes.** Exact path, in order: the test calls `IsolatedHub::shutdown` or drops the guard; `shutdown_inner` creates the `TeardownBudget` and snapshots the owned worker set **while the Hub child is still live**; the `shutdown` command is spawned with `Command::spawn()` and piped stdio, its `ChildStdout` and `ChildStderr` are `take()`n onto one bounded drain thread each, and its retained `Child` is polled with `try_wait()` under phase 1; the Hub child is polled with `try_wait()` under phase 2 while its own pipes drain on their threads. On a normal exit, `try_wait()` returns `Some(status)` for each. On a phase timeout, the path records the typed `TeardownTimeout { phase }`, runs `cleanup_child(&mut child)` for the Hub child or a direct kill for the shutdown command, and reaps the retained `Child`. Either way the path then `join()`s both drain threads and collects their bounded output, runs `reap_owned_session_workers` on the remaining budget, and finally calls `remove_data_dir`. The reap always begins after the Hub process is confirmed dead, which is what closes the spawn race on both paths. The oracle is a live `ps` census taken after the reap returns, asserting zero owned workers and zero owned descendants. The proof drives the real `IsolatedHub` shutdown path, not a helper. Red-on-revert control: with the reap call removed, the same test must observe a surviving worker. Positive control: Implement must additionally show the census returning a **non-empty** owned set on a live Hub before shutdown, because a reap that silently finds nothing is indistinguishable from a reap that works, and that control is what makes the red-on-revert arm meaningful. All four shutdown paths are exercised, and the stalled-command and non-exiting-child tests drive the timeout branches of this same path. |
