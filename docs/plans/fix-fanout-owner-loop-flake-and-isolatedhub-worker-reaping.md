...
| `teardown_bounds` | **Revised on visit 3 after `finding_1787898274_776728`.** One `TeardownBudget` deadline is created on entry to `shutdown` and to `Drop`, and it bounds **every phase**, not only the reaps. The previous revision bounded reaps alone, which left two unbounded blocking waits ahead of them: `shutdown_inner` calls the external shutdown command through `Command::output()` and then calls `child.wait_with_output()`, and neither has a deadline. Because `Drop` calls `shutdown_inner`, `Drop` could hang forever before any reap started. Phases and their transitions: (1) **Shutdown command**, budgeted 5 s. The command runs on a helper thread whose result returns over a channel read with `recv_timeout`, so the child's piped stdout and stderr are still drained and cannot deadlock on a full pipe buffer. On expiry the helper's child is killed and the path records `IsolatedHubError::TeardownTimeout { phase: ShutdownCommand }`, then falls through to phase 3 instead of returning. (2) **Hub-child wait**, budgeted 5 s, using the same drain-on-a-thread plus `recv_timeout` shape for `wait_with_output()`. On expiry the path records `TeardownTimeout { phase: HubChildWait }` and falls through to phase 3. (3) **`cleanup_child`**, which keeps its existing internal bound of about 2.5 s and already signals the Hub child's process group with `SIGTERM` then `SIGKILL`. (4) **Reap**, which receives whatever remains of the budget, with a 10 s ceiling. Within the reap: `SIGTERM` first, `SIGKILL` after 400 ms, a 50 ms sleep between census turns, and an unconditional return when the budget expires even if the census is still non-empty. Repeated reap calls on one path share the one deadline and cannot compound. Worst-case whole-path teardown is bounded at roughly 22.5 s. Every timeout transition is typed and recorded rather than silent, no phase can block forever, and no phase propagates a panic out of `Drop`. |
...
| `crates/botster-hub-test-support/src/isolated_hub.rs` | Capture the Hub-child pid into `IsolatedHub` at spawn so it survives `self.child = None`; add the pgid-scoped owned-worker census, the pre-signal descendant closure, the leader-guarded signal helper, a shared four-phase `TeardownBudget` with a typed `IsolatedHubError::TeardownTimeout { phase }`, drain-on-a-thread plus `recv_timeout` wrappers for the shutdown command and the Hub-child wait, and the bounded idempotent reap; call the reap on all four shutdown paths. No `data_dir_arg` field is added. | Test support |
...
7. **The bounded wait reintroduces a pipe deadlock.** `wait_with_output` exists partly to drain the child's piped stdout and stderr; a naive `try_wait` poll would let a chatty child block on a full pipe buffer. Mitigation: the bounded wait keeps a helper thread doing the real `wait_with_output` and reads its result with `recv_timeout`, so draining continues. Acceptance check 11(c) is the control.
8. **A leaked helper thread after a phase timeout.** A timed-out phase leaves its helper thread blocked on a child that never exits. Mitigation: the timeout path kills that child before moving on, so the helper unblocks and the thread exits; Implement must confirm no helper outlives the teardown.
