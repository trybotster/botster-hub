...
2. The timeout path **re-snapshots that closure immediately before any `cleanup_child` group signal**, so processes spawned since the first snapshot are captured while their links still exist.
...
| `teardown_bounds` | **Revised again on visit 4 after `finding_1787899206_353296`.** One `TeardownBudget` deadline is created on entry to `shutdown` and to `Drop`, and it bounds **every phase**. Visit 3 bounded the right phases but named an unworkable mechanism: `Child::wait_with_output(self)` **consumes** the `Child`, so a helper thread that owns it leaves the teardown path with no `&mut Child` to pass to `cleanup_child`, and `Command::output()` never exposes its `Child` at all. A PID alone cannot restore either. **Corrected mechanism, applied identically to the shutdown command and the Hub child:** spawn each command explicitly with `Command::spawn()` and piped stdio; immediately `take()` the `ChildStdout` and `ChildStderr` and move **only those pipes** into one drain thread each, so the `Child` itself never leaves the teardown owner; poll `Child::try_wait()` against the shared deadline. On timeout, **first re-snapshot the owned worker set and its full descendant closure**, then signal and reap through the **retained** `Child` — `cleanup_child(&mut child)` for the Hub child, which already escalates `SIGTERM` then `SIGKILL` over its process group, and a direct kill plus `wait()` for the shutdown command — then `join()` both drain threads and collect their bounded output. The snapshot must precede the group signal, because that signal reaches the session workers and reparents their PTY children. Reaping before joining is what lets the drain threads finish: each returns when its pipe closes at process exit. Each drain thread caps what it buffers, so a chatty stalled child cannot grow memory without bound while still filling `ShutdownFailed { stderr }` and `DaemonExited { status, stdout, stderr }`. Phases and budgets: (1) shutdown command, 5 s, then `TeardownTimeout { phase: ShutdownCommand }` and fall through to phase 3; (2) Hub-child wait, 5 s, then `TeardownTimeout { phase: HubChildWait }` and fall through to phase 3; (3) `cleanup_child`, existing internal bound of about 2.5 s; (4) reap, remaining budget with a 10 s ceiling, `SIGTERM` first, `SIGKILL` after 400 ms, 50 ms between census turns, unconditional return on expiry. Repeated reap calls share the one deadline and cannot compound. Worst-case whole-path teardown is roughly 22.5 s. Every timeout transition is typed and recorded rather than silent, no phase can block forever, no drain thread outlives the teardown, and no phase propagates a panic out of `Drop`. |
| `late_message_matrix` | **Rewritten on visit 5 for `finding_1787899600_156636`; the previous text still named the removed `wait_with_output` path.** Five ownership-creating surfaces, each with tag, reject, and sweep. (1) *A session worker spawned before shutdown*: tagged by argv0 basename `botster-session-worker` **and** pgid equal to this instance's Hub-child pid; swept by the census loop. (2) *A worker spawned during the shutdown race*: **spawn-race closure differs by exit path, and both are closed before the reap begins.** On the normal path, `Child::try_wait()` returns `Some(status)`, which means the Hub process has already exited and can spawn nothing further. On the timeout path, `try_wait()` never returns `Some` inside the phase budget, so the Hub is still live and could still spawn; closure comes from `cleanup_child(&mut child)`, which signals the Hub child's process group with `SIGTERM` then `SIGKILL` and confirms the group is gone, after which the retained `Child` is reaped. In both cases the owned-worker reap starts only after the Hub process is confirmed dead, so no worker can appear behind the census. A worker that appeared just before either closure still carries the inherited pgid and is caught, because the census re-runs every 50 ms until empty or the budget expires. A process group outlives its dead leader while any member is alive, so pgid identity stays valid after the Hub child exits. (3) *A PTY command descendant of a worker*: **not** in the worker's process group. Core's PTY path makes each session command its own process-group leader, so `killpg` on the Hub group cannot reach it, and neither can `cleanup_child`'s group kill on the timeout path. It is tagged by the `ps` parent/child closure of each owned worker. **Capture ordering is the whole guarantee, and visit 6 corrected it for `finding_1787900107_250035`:** the closure is snapshotted while the workers are still alive, and re-snapshotted immediately **before** any `cleanup_child` group signal. That signal reaches the workers inside the Hub process group; once a worker dies its PTY children are reparented and the ppid link is gone, so a closure computed after the signal would find nothing and the timeout path would silently skip the descendant cleanup the ticket requires. The reap therefore operates on the union of the captured closure and a live re-census, and it signals each captured descendant by PID or through its own leader group, per [[session registry process pid identifies the pty command not the session worker]]. (4) *A worker owned by a different live IsolatedHub or a different worktree*: rejected by the pgid mismatch; never swept. (5) *A zombie worker*: retained in the census for absence proof and never signalled, per [[zombie recovery workers are dead for liveness but remain in absence proof]]. |
| `production_path_proof` | **Rewritten on visit 5, capture ordering corrected on visit 6 for `finding_1787900107_250035`.** Exact path, in order: the test calls `IsolatedHub::shutdown` or drops the guard; `shutdown_inner` creates the `TeardownBudget` and takes an **owned-set snapshot while the Hub child is still live** that records both the owned worker PIDs **and each worker's full `ps` parent/child descendant closure**, because a PID list alone is not enough once links are broken; the `shutdown` command is spawned with `Command::spawn()` and piped stdio, its `ChildStdout` and `ChildStderr` are `take()`n onto one bounded drain thread each, and its retained `Child` is polled with `try_wait()` under phase 1; the Hub child is polled with `try_wait()` under phase 2 while its own pipes drain on their threads. On a normal exit, `try_wait()` returns `Some(status)` for each. On a phase timeout, the path **first re-snapshots the owned worker set and its descendant closure**, so anything spawned since the initial snapshot is captured, and only then records the typed `TeardownTimeout { phase }` and runs `cleanup_child(&mut child)` for the Hub child or a direct kill for the shutdown command, reaping the retained `Child`. **The refreshed closure snapshot must precede every `cleanup_child` group signal**, because that signal reaches the session workers inside the Hub process group and a dying worker's PTY children are reparented, which destroys the ppid links the closure depends on. Either way the path then `join()`s both drain threads and collects their bounded output, runs `reap_owned_session_workers` over the **union of the captured closure and a live re-census** on the remaining budget, and finally calls `remove_data_dir`. The captured set covers processes whose parent links the group signal destroyed; the live re-census covers anything that appeared afterwards. The reap always begins after the Hub process is confirmed dead, which is what closes the spawn race on both paths. The oracle is a live `ps` census taken after the reap returns, asserting zero owned workers and zero owned descendants. The proof drives the real `IsolatedHub` shutdown path, not a helper. Red-on-revert control: with the reap call removed, the same test must observe a surviving worker. Positive control: Implement must additionally show the census returning a **non-empty** owned set on a live Hub before shutdown, because a reap that silently finds nothing is indistinguishable from a reap that works. All four shutdown paths are exercised, and the stalled-command and non-exiting-child tests drive the timeout branches of this same path. |
...
6. **Reparenting destroys descendant links on the timeout path.** `cleanup_child` signals the Hub process group, which contains the session workers, so a worker can die and hand its PTY children to init before the reap computes any closure. A closure computed after that signal finds nothing, and the timeout path silently skips descendant cleanup. Mitigation: snapshot the descendant closure while the workers are alive and refresh it immediately before any group signal, then reap over the union of the captured closure and a live re-census. Acceptance check 11(b) now requires a separate-process-group PTY descendant on the timeout branch, so a too-late capture fails the test.
7. **Core changes the worker's process group.** Assumption 5 is a cross-repository behavioral dependency on the pinned Core revision. Mitigation: Implement re-verifies it against the lockfile revision and records the check; the named fallback is a pre-exit descendant snapshot.
8. **The bounded wait reintroduces a pipe deadlock.** `wait_with_output` exists partly to drain the child's piped stdout and stderr; a bare `try_wait` poll would let a chatty child block on a full pipe buffer. Mitigation, corrected on visit 4: `take()` the `ChildStdout` and `ChildStderr` and drain each on its own thread while the teardown owner keeps the `Child` and polls `try_wait`. Draining continues and the `Child` stays available for `cleanup_child`. Acceptance check 11(c) is the control.
9. **A leaked drain thread after a phase timeout.** A timed-out phase can leave a drain thread blocked reading a pipe that never closes. Mitigation: the timeout path signals and reaps the child through the retained `Child` **before** it joins, so each pipe closes at process exit and both drain threads return. Acceptance check 11(d) requires proof that every drain thread exits after a timeout, and Implement must confirm no drain thread outlives the teardown.
10. **Reaping masks a real Hub leak.** A test-support reap can hide a genuine failure to stop workers. Mitigation: [[hub shutdown preserves durable session workers]] records that Hub stop intentionally preserves durable workers, so the reap is fixture hygiene rather than a cover for a Hub defect. Review must confirm the reap is not called in place of an assertion that a production path already owns.
11. **`Drop` panics.** A panic in `Drop` during unwinding aborts. Mitigation: the reap swallows every error and the existing `thread::panicking()` guards stay.
12. **The gate stays flaky for a third cause.** Two causes are now known. A third may exist. Mitigation: the gate must pass at default workspace concurrency without a waiver, and isolated reruns do not count as proof.
13. **Strict Clippy hides later diagnostics.** Per [[strict clippy can hide later crate diagnostics behind the first compile failure]], one green run after a repair is not proof. Mitigation: rerun the full workspace Clippy after every repair.
14. **Wrong toolchain in the pipeline shell.** Per [[botster Hub pipeline shells can override RUSTUP TOOLCHAIN below the CI pin]], a bare strict Clippy can exit 0 on unfixed code. Mitigation: `RUSTUP_TOOLCHAIN=1.97.0` on every gate command, with `rustc --version` recorded from that shell.
...
10. **Capture a process closure before the signal that breaks it.** A group kill that reaches a parent reparents its children to init, so any parent/child closure computed afterwards is empty. Cleanup that must reach processes outside the signalled group has to snapshot the closure while the parents are alive, and refresh it immediately before the signal. Candidate note: "snapshot a descendant closure before the signal that reparents it".
