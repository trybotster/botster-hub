...
- **Repair rows: 11.** The table below has 11 rows. Row 8 groups two matching lines that live in one file, belong to one relocation, and take one repair. Every other row is one matching line. This is the number of repair units, not the number of grep hits.

Each row belongs to exactly one relocation, named in the Pair column, because a row is repaired inside that relocation's own import-repair commit. Round 4 of this plan grouped `event_plane_saturation.rs` lines 126 and 176 into one row, which was wrong: line 126 names the entity module and line 176 names the attach module, so the two lines belong to different pairs and cannot share a commit. They are rows 10 and 11 below.
...
| # | Pair | Site | Reference | Fails at | Repair |
|---|---|---|---|---|---|
| 1 | attach | `src/daemon_attach_stream.rs:1129` | `include_str!("daemon_attach_stream.rs")` self-scan inside `attach_stream_source_does_not_branch_on_snapshot_phases` | compile | `include_str!("attach_routes.rs")` |
| 2 | entity | `src/lib.rs:1007` | `include_str!("daemon_entity_subscriptions.rs")` | compile | `include_str!("subscription/entity.rs")` |
| 3 | attach | `src/lib.rs:1011` | `include_str!("daemon_attach_stream.rs")` | compile | `include_str!("subscription/attach_routes.rs")` |
| 4 | attach | `src/daemon_transport.rs:147` | `#[path = "daemon_attach_stream.rs"]` | compile | removed with the module declaration |
| 5 | entity | `src/daemon_transport.rs:166` | `#[path = "daemon_entity_subscriptions.rs"]` | compile | removed with the module declaration |
| 6 | entity | `src/lib.rs:1006` | display string `"src/daemon_entity_subscriptions.rs"` | test message only | `"src/subscription/entity.rs"` |
| 7 | attach | `src/lib.rs:1010` | display string `"src/daemon_attach_stream.rs"` | test message only | `"src/subscription/attach_routes.rs"` |
| 8 | entity | `tests/session_projection_owner_loop.rs:176` and `:191` | scan-list entry plus its exclusion comparison, read through `fs::read_to_string(root.join(relative))` | test | `"src/subscription/entity.rs"` in both places |
| 9 | attach | `tests/hub_daemon_lifecycle/subscription_ownership_baseline.rs:647` | `hub_source("src/daemon_attach_stream.rs")` | test | `hub_source("src/subscription/attach_routes.rs")` |
| 10 | entity | `tests/hub_daemon_lifecycle/event_plane_saturation.rs:126` | scan-list entry read through `fs::read_to_string(root.join(relative))` | test | `"src/subscription/entity.rs"` |
| 11 | attach | `tests/hub_daemon_lifecycle/event_plane_saturation.rs:176` | direct `fs::read_to_string(root.join("src/daemon_attach_stream.rs"))` | test | `"src/subscription/attach_routes.rs"` |
...
|---|---|---|---|---|---|
| `Attach` | `AttachStreamRegistry.streams`, `active_subscriptions`, `attach_owner_grant_ids`, `connection_bound_routes` in `src/subscription/attach_routes.rs` | `AttachStreamOwner.grant_id` for WebRTC; the connection id for Unix | Pre-READY attach failure creates no route and increments no lifecycle count | Route-aware idempotent cleanup keyed on route identity; cannot decrement another route | **Relocation plus its own import repair,** as the Scope section defines those two kinds. The file moves byte-pure; the paired commit carries the full import-repair scope for this file, which for `attach_routes.rs` is path-reference repair rows 1, 3, 4, 7, 9, and 11. No line of admission or cleanup logic changes |
