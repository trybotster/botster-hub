...
- Bound `capture_new_session_workers_for_data_dir` (`process.rs:389-463`) to registry-backed verified ancestry: load `SessionRegistry` for the data dir, walk each registered command PID to a worktree `botster-session-worker` ancestor, and exclude `before_pids`. Do not match workers by argv data-dir string (real worker argv omits it) and do not adopt any live worker from this worktree.
...
*Per-failure typed markers.* Budget-expiry paths in the shared wait helpers emit a structured, machine-parseable marker line in the panic message: `harness_budget_expired test=<rust test name> kind=<wait-kind> budget_ms=<n> resource=<class or none> probe=<confirmed|unconfirmed|n/a>` plus captured evidence. The `test` field is the last `::` segment of `thread::current().name()` (the rustc test name). Classification of the `resource` field:
...
2. `product_failure` — any failed test whose panic does **not** carry a matching `harness_budget_expired test=<that rust test name>` marker, carries a matching marker with `resource=none`, or carries an ambiguous resource without `probe=confirmed`, or any test that fails a semantic assertion. A host marker that names a different test does not cover this failure. A product assertion always outranks host pressure: high ambient load never reclassifies a semantic failure.
3. `host_exhaustion` — every failed test has its own matching named marker with unambiguous resource evidence or `probe=confirmed`. Global marker presence is not enough.
...
- Host evidence captured during the run (04:02:52Z): load average `{22.54 31.30 53.28}` (earlier `{33.50 34.09 55.39}`), and **226 botster-role processes host-wide**, attributed by path: 73 from the ambient botster-hub checkout, 64 from worktree `ticket_1786661010_198387`, 38 from `ticket_1786661008_634435`, 15 from `ticket_1786937228_425608`, 11 from `ticket_1786977409_499180`, 10+8+2+2 from four further ticket worktrees — and only 2–4 from this worktree. One leftover PTY producer shell carried a data-dir nanos stamp from 2026-08-11.
