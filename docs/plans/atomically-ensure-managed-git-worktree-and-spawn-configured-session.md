...
3. Reuse the durable `Worktree` registry for successful managed worktrees, with an additive serde-defaulted management/provenance discriminator (`registered` versus `hub_managed_git`).
   ...
   - Deliberately change the registry invariant only for `hub_managed_git` rows: they live under the explicit Hub data-directory managed root and are exempt from generic target-root containment. Startup adoption and the bounded managed-Git lane verify the exact deterministic path, Git common-dir identity, and branch ownership, then persist `present`, `missing`, or `stale`; list/show project that last persisted result without executing Git on the Hub owner thread. Generic `registered` rows retain today's target-root containment and status behavior.
