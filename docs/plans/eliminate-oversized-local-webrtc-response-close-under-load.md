...
3. Repair flow-control ownership at the channel loop, not in framing or daemon response production. Pressure state must survive response boundaries; a completed final chunk must not be reclassified as a partial response merely because high water is observed afterward. Before any later unsent chunk/response is enqueued, the same bounded high/low-water state must still gate delivery. Keep the existing fixed thresholds and absolute five-second bound for genuinely pending delivery, preserve the bounded inbound FIFO, and route real send/close failures through the existing idempotent peer cleanup. The Implementer must use the captured failing state to confirm or adjust this exact branch; if evidence instead identifies peer-connection closure outside the sender state machine, stop and return the finding rather than applying the hypothesis silently.
4. Strengthen the real WebRTC test so it proves both byte-exact oversized reassembly and continued usability of the same data channel with a small request after the large response. Preserve the 300,000-byte payload assertion, multi-chunk assertion, frame ceiling, ordered/reliable channel, and cleanup checks.
...
- Assumption to prove, not silently treat as fact: because the large response's final assertions passed and only the following request saw closure, the likely defect is response-boundary flow-control state causing sender teardown after the final chunk, rather than malformed framing or lost response bytes.
...
- Unknown: deterministic negative proof may require a small fake-channel scheduling seam so a high-water event can be placed after the final frame without wall-clock sleeps. That seam belongs in `src/local_webrtc.rs` tests and must not become runtime configuration.
...
- `src/local_webrtc.rs` — channel-owned pressure/delivery state, explicit send outcome/diagnostics, response-boundary behavior, cleanup integration, and deterministic fake-channel regression tests.
...
1. Deterministic `src/local_webrtc.rs` tests prove:
   ...
   - the pressure state is retained and gates the next response/request until low water, without resetting its deadline;
...
- Both `project_pipelines_create_vault_checklist` and `project_pipelines_create_checklist` timed out through the plugin worker. Per [[project pipelines checklist worker timeouts require artifact evidence fallback]], the required checklist evidence is preserved in this plan, the attached artifact, gate evidence, and step-advance evidence. Checklist creation may be retried once separately, but repeated worker calls must not block the plan.
