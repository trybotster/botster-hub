...
`src/daemon/control/request.rs` -- `ControlMessage::Request` owner. Owns the universal WebRTC live-peer gate, event and host early paths, `handle_control_request` post-processing (occupancy overlay, pump marks, drain owner snapshot, cleanup accounting, shutdown-busy Hub update reply), and the single reply. `handle_control_message` delegates this variant once. Review `finding_1788039873_333814` required this dedicated owner so the Request arm is not a second dispatcher body.
...
`src/daemon/control/events.rs` -- `SubscribeEvents`, `UnsubscribeEvents`, `handle_client_event_request`, and `events_from_client`. Note the precise state, because it is easy to overstate: `events_from_client` is a one-line `map` wrapper. The real mapper, `daemon_event_from_client`, already lives in `src/client_api_dto/session.rs`, which decomposition 1 moved and which `src/lib.rs:1089` already scans. The `DaemonEvent::*` forbidden-construct entries are therefore already protected at their true site, and this ticket does not move that site. `control/events.rs` still enters the guard list, on the same rule that every new `src/daemon/**` file does, not because it carries the mapper.
...
|---|---|---|---|---|---|
| `IssueLocalWebrtcBootstrap` | **yes.** It issues the grant that every later WebRTC ownership row is keyed on | `control/webrtc.rs` dispatch; the registry stays in `src/admission/grants.rs`, which this ticket does not move | the grant binds `(grant_id, grant_secret, expected_origin)` with `expires_at = now + GRANT_TTL_SECONDS` (120) and `redeemed: false`; the origin must equal the running entrypoint's `local_url` origin | six typed `local_webrtc_bootstrap_*` legs reject unsupported entrypoint, uninstalled package, disabled package, missing entrypoint, non-running entrypoint, and origin mismatch; no grant is issued on any failing leg, per [[webrtc bootstrap origin must be requested after the package server binds]] | **the grant row is never swept by `PeerClosed`.** Peer removal touches `peers`, `peer_states`, and `stale_close_peers` only. The row is marked `redeemed = true` at signal time and stays in `GrantRegistry` until the next `issue_bootstrap` call prunes it past its 120-second expiry, or `stop_all` clears the registry at daemon shutdown. Retention is a **replay diagnostic**, not a reuse defense: a removed row would still reject a replay, but with `MissingGrant` instead of `RedeemedGrant`. Keeping a live redeemed row preserves that distinction, which is what `issuing_bootstrap_prunes_expired_grants_and_keeps_live_replay_diagnostics` pins by dropping an expired redeemed grant while keeping a live one |
| `LocalWebrtcSignal` | **yes, and it is the largest ownership creation in this file.** It redeems the grant and creates the live `RTCPeerConnection`, its peer state, its data-channel task, and its entries in the peer maps | `control/webrtc.rs` dispatch; mechanics stay in `src/transport/webrtc/{signaling.rs,peer.rs}` and `src/admission/grants.rs`, none of which this ticket moves | `GrantRegistry::redeem` validates `(grant_id, grant_secret, origin)` before any peer exists | `GrantRegistry::redeem` returns `MissingGrant` when the row is absent; otherwise `validate` runs a fixed order of `RedeemedGrant`, `ExpiredGrant`, `SecretMismatch`, `OriginMismatch`. A grant is single-use from the moment of its first successful redeem, independent of any peer: every later signal for that grant id fails and creates no second peer. `redeemed` is checked before `expires_at`, so an expired but still-present redeemed row returns `RedeemedGrant`, and only a pruned row returns `MissingGrant`. This order is pinned by `grant_validation_runs_redeemed_expiry_secret_then_origin` | stage-specific, because `LocalWebrtcTransport::signal` redeems before it builds anything. **Pre-redeem failure** (`MissingGrant`, or any `validate` rejection) leaves the row unchanged and creates no peer. **Post-redeem failure** (`secret_stream_key` inside `redeem`, then `self.runtime()?`, then `block_on(answer_offer(...))?`) leaves the row marked `redeemed = true` with no peer created: the grant is burned and the browser must request a new bootstrap. **Success** inserts into `peers` and `peer_states`, and that peer is the ownership set `LocalWebrtcPeerClosed` later sweeps. The burn point is the `grant.redeemed = true` assignment in `redeem`, which precedes both the stream-key derivation and every peer-construction step |
...
| `Attach` | yes | `control/sessions.rs`; sweep in `control/webrtc.rs` | `(session_id, subscription_id)` plus `attach_owner_grant_ids` keyed by `grant_id` | covered by the universal grant-tagged gate above. Be precise about the second filter: `response_records_attach_ownership` (`src/subscription/attach_routes.rs:924`) returns false for `DaemonResponseKind::OperatorError` **only**. An `attach_failed` outcome that is not an `OperatorError` still records the row here; the production comment at `src/transport/webrtc/control_channel.rs:550` names a separate Drain path that releases the failed route so `PeerClosed` does not send `Detach` for it. The move must preserve both mechanisms separately | `detach_local_webrtc_subscriptions` sweeps the `LocalWebrtcPeerClosed` snapshot; `attach_owner_grant_ids.retain` drops only rows owned by a removed grant |
...
`ownership_identity`: `grant_id` identifies a WebRTC peer, and a grant is single-use from its first successful redeem. `GrantRegistry::redeem` sets `redeemed = true` and keeps the row, so every later `LocalWebrtcSignal` for that grant id is rejected and can never produce a second peer under the same id. This is independent of peer lifecycle: the rejection holds whether the first peer is still live, has closed, or was never fully established. The grant map and the peer map are independent, and `has_live_peer` reads `peers`, never `grants`. `client_id` identifies a Unix connection. `(session_id, subscription_id, generation)` identifies a terminal route. A delayed `LocalWebrtcPeerClosed` snapshot cannot delete a row now owned by a live replacement peer, because every removal is gated on the row still being unowned or owned by a removed grant. Both the identity fields and the gating logic move unchanged into `src/daemon/control/webrtc.rs`.
...
16. Every new `src/daemon/**` file appears in the `src/lib.rs` `production_sources_reject_terminal_drain_and_snapshot_phase_decode` list, including `src/daemon/control.rs`, `src/daemon/control/message.rs`, and `src/daemon/control/packages/mutations.rs`. `src/daemon/control/events.rs` is included on that same every-new-file rule. It carries only the `events_from_client` wrapper; the real `DaemonEvent` construction site, `daemon_event_from_client` in `src/client_api_dto/session.rs`, is already scanned at `src/lib.rs:1089` and does not move in this ticket.
...
5. **A Hub bootstrap grant is retained after redemption to keep a replay diagnostic, not to prevent reuse.** `GrantRegistry::redeem` sets `redeemed = true` and keeps the row; peer close never removes it; the registry shrinks only through the lazy `prune_expired_grants` at the next `issue_bootstrap` and is cleared wholesale only by `stop_all`. `prune_expired_grants` drops an expired redeemed grant while keeping a live one, so a replay against a live row returns `RedeemedGrant` and a replay against a pruned row returns `MissingGrant`. Both reject, so removing the row early is not a security hole; it erases an observable typed distinction. The precise value of the retained row is easy to overstate in either direction, and the vault records neither. Capture after acceptance check 4c lands.
