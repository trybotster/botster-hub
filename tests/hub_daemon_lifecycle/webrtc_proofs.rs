...
fn webrtc_blind_shutdown_after_finite_write(
    hub_name: &str,
    session_id: &str,
    wrapper: SessionWorkerWrapperKind,
) -> (botster_hub_client::DaemonResponse, PathBuf, botster_hub_test_support::IsolatedHub)
{
    let expected: &[u8] = &[0x00, 0x1b, 0xff, 0xc0];
    let hub = start_isolated_live_output_hub_with_wrapper(hub_name, wrapper);
    let package_dir = unique_test_dir(&format!("{hub_name}-web"));
    write_botster_web_package(&package_dir);
    enable_supervised_package(hub.data_dir(), &package_dir);
    let endpoint = hub.endpoint().clone();
    let web_listener_port = unused_loopback_port();
    ...
    block_on(async {
        ...
        let release_path = unique_short_test_dir(&format!("{hub_name}-rel")).join("go");
        let script_path = write_python_wait_then_write_script(&release_path, expected);
        ...
        assert_eq!(
            ...
            "wrapper worker spawn must succeed, got kind={:?} error={:?}",
            spawned.kind,
            spawned.error
        );
        offer_peer
        ...
        let attach = offer_peer
            .encrypted_request(
                ...
                &botster_hub_client::DaemonRequest::Attach {
                    session_id: session_id.to_string(),
                    subscription_id: format!("{session_id}-sub"),
                },
            )
            .await
            .expect("attach over encrypted WebRTC");
    ...
    .expect("blind ShutdownSession after natural write(2) exit");
    (shutdown, hub.data_dir().clone(), hub)
}

#[test]
#[ignore = "blocked on ticket_1787015956_494734 Core ProcessExited delivery"]
fn external_hub_webrtc_shutdown_after_w1_delayed_worker_reap_is_idempotent() {
    let _guard = daemon_test_guard();
    let session_id = "webrtc-w1-session";
    let (shutdown, data_dir, hub) = webrtc_blind_shutdown_after_finite_write(
        "webrtc-w1",
        session_id,
        SessionWorkerWrapperKind::DelayedReap,
    );
    eprintln!(
        "PHASE1 W1 WebRTC shutdown kind={:?} error={:?}",
        shutdown.kind, shutdown.error
    );
    assert_shutdown_strict_natural_exit(&shutdown, session_id, "W1 WebRTC delayed reap");
    hub.shutdown().expect("shutdown isolated hub");
    reap_session_workers_for_data_dir(&data_dir)
        .expect("W1 WebRTC hub shutdown must not leave worktree session workers");
}

#[test]
#[ignore = "blocked on ticket_1787015956_494734 Core ProcessExited delivery"]
fn external_hub_webrtc_shutdown_after_w2_nonsuccess_worker_is_idempotent() {
    let _guard = daemon_test_guard();
    let session_id = "webrtc-w2-session";
    let (shutdown, data_dir, hub) = webrtc_blind_shutdown_after_finite_write(
        "webrtc-w2",
        session_id,
    ...
    eprintln!(
        "PHASE1 W2 WebRTC shutdown kind={:?} error={:?}",
        shutdown.kind, shutdown.error
    );
    assert_shutdown_strict_natural_exit(&shutdown, session_id, "W2 WebRTC non-success worker");
    let listed =
        botster_hub_client::request(hub.endpoint(), botster_hub_client::DaemonRequest::ListSessions)
            .expect("list after W2 WebRTC shutdown");
    assert!(
        ...
        "W2 WebRTC must reach a terminal lifecycle on the control plane: {:?}",
        listed.sessions
    );
    hub.shutdown().expect("shutdown isolated hub");
    reap_session_workers_for_data_dir(&data_dir)
        .expect("W2 WebRTC hub shutdown must not leave worktree session workers");
}
